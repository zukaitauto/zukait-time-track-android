...
assert.match(job,/order by j\.updated_at desc, j\.job_card desc limit greatest\(1,least\(coalesce\(p_limit,100\),500\)\)/);
...
assert.match(api,/const nextCursor = rows\.length === limit && cursorValue && cursorId \? \{ before:String\(cursorValue\), before_id:String\(cursorId\) \} : null/);
