...
ctx.me={id:'S1',role:'Supervisor'};ctx.render();assert.equal(tile.className,'cons-supervisor-tile');ctx.render();assert.ok(tile);
...
ctx.me={id:'M1',role:'Manager'};ctx.consRunReports();assert.match(element('crResult').innerHTML,/2.25 Litre/);assert.match(element('crResult').innerHTML,/OMR 7.875/);
...
assert.throws(()=>C.managerCorrectActual(state,state.consumables.actuals[0].id,[{materialId:m.id,brandId:b.id,quantity:1}],ctx.me,''),/REASON_REQUIRED/);assert.equal(JSON.stringify(state),before);
assert.throws(()=>C.managerCorrectIssue(state,state.consumables.issues[0].id,{colourCode:'changed'},ctx.me,''),/REASON_REQUIRED/);assert.equal(JSON.stringify(state),before);
