...
assert.match(workflow,/function notifySupervisor/);assert.match(main,/function denterView/);assert.match(main,/Notify Supervisor/);assert.match(main,/const add=\(denterView\(\)\|\|role\(\)==='Purchaser'\)\?'':'<hr>/,'Denter and Purchaser must not see Supervisor part-entry controls');assert.match(main,/function canManage\(\).*Manager.*Supervisor/,'Supervisor list-edit authority missing');assert.doesNotMatch(main,/function canManage\(\).*Purchaser/,'Purchaser must not have Supervisor list-edit authority');assert.match(main,/function canPurchase\(\).*Manager.*Purchaser/,'Purchaser commercial authority missing');assert.match(main,/function addItem\([^)]*\)\{if\(!canManage\(\)\)return \{ok:false,reason:'FORBIDDEN'\}/,'direct addItem must reject Denter and Purchaser');assert.match(main,/async function createFromUI\(\)\{if\(!canManage\(\)\)/,'UI create must reject Denter and Purchaser');
