...
let bad=p.lists.create({jobCard:'JC-Q',lines:[{name:'Bumper',qty:1.5}]},{role:'Supervisor'});assert.equal(bad.code,'INVALID_QTY');bad=p.lists.create({jobCard:'JC-Q',lines:[{name:'Bumper',qty:0}]},{role:'Supervisor'});assert.equal(bad.code,'INVALID_QTY');const noPrice={number:'PL-NP',view:'WAITING',lines:[{id:'N',name:'Lamp',qty:1,status:'SUPERVISOR_CONFIRMED',arrivedQty:1,acceptedQty:1,finalPriceOMR:null}]};assert.notEqual(p.lists.deriveView(noPrice),'PURCHASE_COMPLETED');
