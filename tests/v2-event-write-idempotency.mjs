...
assert.match(sql,/event_id text primary key/i);assert.match(sql,/on conflict (?:\(event_id\)|on constraint workshop_v2_events_pkey) do nothing/i);assert.match(sql,/zukait_v2_commit_event/i);assert.match(sql,/grant execute[^;]+service_role/is);assert.match(sql,/revoke all[^;]+anon,authenticated/is);
