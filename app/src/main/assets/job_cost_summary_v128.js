...
function managerHtml(no){
 ...
 return '<section class="v128-job-cost"><div class="v128-cost-head"><div><b>Job Cost & Material Status</b><small>Live Job Card costing summary</small></div></div><div class="v128-status-grid">'+pill('CONSUMABLES',d.consumablesStatus,'materials')+pill('PAINT',d.paintStatus,'paint')+'</div><div class="v128-cost-grid">'+pill('LABOUR COST','OMR '+d.labour.toFixed(3),'labour')+pill('CONSUMABLES COST','OMR '+d.materials.toFixed(3),'materials')+pill('PAINT COST','OMR '+d.paint.toFixed(3),'paint')+pill('TOTAL JOB COST','OMR '+d.total.toFixed(3),'total')+'</div></section>';
}
