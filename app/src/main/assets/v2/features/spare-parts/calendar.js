...
function dayKey(v){const d=new Date(v);return d.getUTCFullYear()+'-'+String(d.getUTCMonth()+1).padStart(2,'0')+'-'+String(d.getUTCDate()).padStart(2,'0')}
function workshopDaysBetween(from,to,{holidayKeys=[],fridayDay=5}={}){let a=new Date(from),b=new Date(to);if(!Number.isFinite(+a)||!Number.isFinite(+b)||b<=a)return 0;a=new Date(Date.UTC(a.getUTCFullYear(),a.getUTCMonth(),a.getUTCDate()));b=new Date(Date.UTC(b.getUTCFullYear(),b.getUTCMonth(),b.getUTCDate()));const h=new Set(holidayKeys);let n=0;for(let d=new Date(a.getTime()+86400000);d<=b;d=new Date(d.getTime()+86400000)){if(d.getUTCDay()!==fridayDay&&!h.has(dayKey(d)))n++}return n}
