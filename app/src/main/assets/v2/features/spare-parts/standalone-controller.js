...
const x=state.active&&findPL(state.active);if(x&&p.completion){const applied=p.completion.apply(x);Object.assign(x,applied.list);if(!x.view)x.view=x.deliveredPending?'DELIVERED_PENDING':'WAITING';if(!x.view)x.view=x.deliveredPending?'DELIVERED_PENDING':'WAITING'}return{ok:true,state:snapshot()}}
