...
function setShop(q,slot,name){if(!slots.includes(slot))return{ok:false,code:'INVALID_SLOT'};const next=structuredClone?structuredClone(q||empty()):JSON.parse(JSON.stringify(q||empty()));next.shops=next.shops||{};next.shops[slot]=String(name||'').trim();return{ok:true,quotation:next}}
function setPrice(q,partId,slot,amount,currency='OMR',rateToOMR=1){if(!slots.includes(slot)||!p.currencies?.includes(currency))return{ok:false,code:'INVALID_QUOTE'};const a=Number(amount),r=Number(rateToOMR);if(!Number.isFinite(a)||a<0||!Number.isFinite(r)||r<=0)return{ok:false,code:'INVALID_AMOUNT'};const next=JSON.parse(JSON.stringify(q||empty()));next.prices=next.prices||{};next.prices[partId]=next.prices[partId]||{};next.prices[partId][slot]={amount:p.money?p.money(a):a,currency,rateToOMR:r,omrEquivalent:p.money?p.money(a*r):a*r};return{ok:true,quotation:next}}
