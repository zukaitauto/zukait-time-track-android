...
function splitCost(lines=[]){let initial=0,additional=0;(lines||[]).filter(x=>x&&!x.voided).forEach(x=>(x.additional?additional:initial)+=Number(x.finalPriceOMR)||0);return{initial:m(initial),additional:m(additional),total:m(initial+additional)}}
