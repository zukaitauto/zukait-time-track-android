...
const signed=v=>Math.round((Number(v)||0)*1000)/1000;
...
function cap(capOMR,currentOMR){const c=Math.max(0,Number(capOMR)||0),v=Math.max(0,Number(currentOMR)||0);return{cap:m(c),current:m(v),balance:m(Math.max(0,c-v)),overBy:m(Math.max(0,v-c)),exceeded:c>0&&v>c,ratio:c>0?v/c:null}}
...
function priceVariance(quotedOMR,finalOMR){const q=Number(quotedOMR)||0,f=Number(finalOMR)||0;return{quoted:m(q),final:m(f),difference:signed(f-q)}}
