...
function finalTotal(lines=[]){return m(lines.filter(x=>x&&!x.voided).reduce((s,x)=>s+(Number(x.finalPriceOMR)||0),0))}
function splitCost(lines=[]){let initial=0,additional=0;(lines||[]).filter(x=>x&&!x.voided).forEach(x=>{const v=Number(x.finalPriceOMR)||0;if(x.additional)additional+=v;else initial+=v});return{initial:m(initial),additional:m(additional),total:m(initial+additional)}}
...
p.analytics={finalTotal,splitCost,cap,period,byJobCard,priceVariance};
})();