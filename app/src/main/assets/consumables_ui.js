...
function deptCard(title,icon,enabled,fn){
 return '<button class="cons-dept '+(enabled?'':'disabled')+'" '+(enabled?'onclick="'+fn+'"':'disabled')+'><span>'+icon+'</span><b>'+title+'</b><small>'+(enabled?'Open module':'Module ready for future setup')+'</small></button>';
}
window.openConsumablesModule=function(){
 if(!['Supervisor','Manager'].includes(role()))return;
 modal('Consumables','<div class="cons-depts">'+deptCard('Painting Consumables','🎨',true,'openPaintingConsumables()')+deptCard('Denting Consumables','🛠️',true,"openConsumablesPlaceholder('Denting')")+deptCard('Mechanical Consumables','⚙️',true,"openConsumablesPlaceholder('Mechanical')")+'</div>');
};
...
window.consAddLine=function(){
 ...
 if(!mid||!bid||!(q>0))return alert('Select Material, Brand and enter Quantity.');
...
window.consFinishIssue=function(){
 ...
 try{
  ...
  const x=C().issue(state,{clientRequestId:draft.clientRequestId,jobCard:draft.jc.job.no,vehicle:draft.jc.vehicle||'',colourCode:document.getElementById('consColour')?.value||'',mainPainterId:painter,allottedSupervisorId:draft.jc.supervisorId||me?.id||'',lines:draft.lines},{id:me?.id||'',role:role()},draft.type==='additional'?C().TYPES.ADDITIONAL:C().TYPES.ISSUED);
  if(typeof save==='function')save(); closeModal(); alert((draft.type==='additional'?'Additional':'Suggested / Issued')+' Materials saved and locked.');
...
window.consLoadActual=function(){
 ...
 if(!d){if(out)out.innerHTML='<div class="notice">Search a Job Card to load Suggested + Additional materials.</div>';return}
 ...
 draft.lines=Object.keys(a).map(k=>{const [materialId,brandId]=k.split('|');return {materialId,brandId,quantity:a[k]};});
...
window.consRenderActualRows=function(){
 ...
 out.innerHTML=draft.lines.length?'<div class="cons-table"><table><tr><th>No.</th><th>Material</th><th>Brand</th><th>Total Issued</th><th>Actual Qty</th></tr>'+draft.lines.map((l,i)=>{const m=c.materials.find(x=>x.id===l.materialId)||{},b=c.brands.find(x=>x.id===l.brandId)||{},allow=C().allowance(state,draft.jc.job.no)[l.materialId+'|'+l.brandId]||0;return '<tr><td>'+(i+1)+'</td><td>'+esc(m.name||'')+'</td><td>'+esc(b.name||'')+'</td><td>'+esc(allow)+' '+esc(m.unit||'')+'</td><td><input class="cons-actual-qty" data-i="'+i+'" type="number" min="0" max="'+esc(allow)+'" step="any" value="'+esc(l.quantity)+'"> '+esc(m.unit||'')+'</td></tr>'}).join('')+'</table></div>':'<div class="notice">No issued material lines found.</div>';
};
window.consFinishActual=function(){
 ...
 if(!confirm('Finish Actual Materials? This will lock Supervisor editing and calculate material cost using the applicable historical price.'))return;
 try{
  draft.clientRequestId=draft.clientRequestId||('ui-'+Date.now()+'-'+Math.random().toString(36).slice(2,10));
  const x=C().finishActual(state,{clientRequestId:draft.clientRequestId,jobCard:draft.jc.job.no,lines},{id:me?.id||'',role:role()});
  if(typeof save==='function')save();closeModal();alert('Actual Materials finished. Total material cost: OMR '+Number(x.totalCost||0).toFixed(3));
 }catch(e){alert(String(e?.message||e))}
};
window.openConsumablesSearch=function(){
 if(!['Supervisor','Manager'].includes(role()))return;
 modal('Search Material List','<div class="cons-entry-shell"><div class="cons-searchbar"><input id="consSearchJc" placeholder="Search Job Card No." oninput="consShowSearch()"><button class="blue" onclick="consShowSearch()">SEARCH</button></div><div id="consSearchResult" class="notice">Enter a Job Card number to view material history.</div></div>');
};
window.consShowSearch=function(){
 ...
 const keys=[...new Set([...Object.keys(allow),...(actual?.lines||[]).map(x=>x.materialId+'|'+x.brandId)])];
 const u=id=>(users||[]).find(x=>x.id===id)?.name||id||'—',mat=id=>c.materials.find(x=>x.id===id)||{},brand=id=>c.brands.find(x=>x.id===id)||{};
 let rows=keys.map((k,i)=>{const [mid,bid]=k.split('|'),m=mat(mid),b=brand(bid),al=actual?.lines?.find(x=>x.materialId===mid&&x.brandId===bid),aq=allow[k]||0;return '<tr><td>'+(i+1)+'</td><td>'+esc(m.name||'')+'</td><td>'+esc(b.name||'')+'</td><td>'+esc(aq)+' '+esc(m.unit||'')+'</td><td>'+esc(al?m.name||'':'—')+'</td><td>'+esc(al?b.name||'':'—')+'</td><td>'+esc(al?al.quantity:'—')+(al?' '+esc(m.unit||''):'')+'</td></tr>'}).join('');
 let history=adds.length?'<details><summary>Additional Materials History ('+adds.length+')</summary><div class="cons-add-history">'+adds.map((x,i)=>'<div><b>Additional #'+(i+1)+'</b> · '+new Date(x.createdAt||0).toLocaleString()+' · '+esc(u(x.createdBy))+'<br>'+x.lines.map(l=>esc(mat(l.materialId).name||'')+' / '+esc(brand(l.brandId).name||'')+' — '+esc(l.quantity)+' '+esc(mat(l.materialId).unit||'')).join('<br>')+'</div>').join('')+'</div></details>':'';
 out.innerHTML='<div class="cons-summary"><b>JC '+esc(d.job.no)+'</b><span>'+esc(d.job.vehicle||'—')+'</span><span>Colour: '+esc(base?.colourCode||'—')+'</span><span>Main Painter: '+esc(u(base?.mainPainterId))+'</span><span>Supervisor: '+esc(u(base?.allottedSupervisorId))+'</span></div><div class="cons-table"><table><tr><th>No.</th><th>Suggested Material</th><th>Brand</th><th>Quantity</th><th>Actual Material</th><th>Brand</th><th>Quantity</th></tr>'+rows+'</table></div>'+history+'<div class="cons-export-actions"><button class="secondary" onclick="consPrintSearch(\'issued\')">PRINT / PDF ISSUED</button><button class="secondary" onclick="consPrintSearch(\'actual\')">PRINT / PDF ACTUAL</button><button class="blue" onclick="consPrintSearch(\'both\')">PRINT / PDF BOTH</button><button class="blue" onclick="consShareSearch()">SHARE / WHATSAPP</button></div>';
};
window.consPrintSearch=function(mode){
 ...
 w.document.write('<html><head><title>Painting Consumables</title><style>body{font-family:Arial;padding:24px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #bbb;padding:8px;text-align:left}button,.cons-export-actions{display:none}</style></head><body><h2>Painting Consumables — '+esc(mode.toUpperCase())+'</h2>'+copy.innerHTML+'</body></html>');w.document.close();w.focus();w.print();
};
...
window.consHistoryCorrectActual=function(id){if(role()!=='Manager')return;const c=C().ensureState(state),a=c.actuals.find(x=>x.id===id&&!x.voided);if(!a)return;const lines=a.lines.map(l=>{const m=c.materials.find(x=>x.id===l.materialId)||{},b=c.brands.find(x=>x.id===l.brandId)||{};const q=prompt((m.name||'Material')+' / '+(b.name||'Brand')+' actual quantity',String(l.actualQuantity));if(q===null)throw new Error('CANCEL');return {materialId:l.materialId,brandId:l.brandId,quantity:Number(q)}});const reason=prompt('Reason for correction (required)');if(reason===null)return;try{C().managerCorrectActual(state,id,lines,me,reason);save();consOpenHistory();alert('Actual Materials corrected and audit history updated.')}catch(e){if(e.message!=='CANCEL')alert(e.message)}};
...
window.consRunReports=function(){
 ...
 const rows=[];actuals.forEach(a=>{const base=c.issues.find(x=>x.jobCard===a.jobCard&&x.type===C().TYPES.ISSUED&&!x.voided);a.lines.forEach(l=>{if((mid&&l.materialId!==mid)||(bid&&l.brandId!==bid))return;const m=c.materials.find(x=>x.id===l.materialId)||{},b=c.brands.find(x=>x.id===l.brandId)||{};rows.push({date:day(a.actualAt),jc:a.jobCard,material:m.name||'',brand:b.name||'',unit:m.unit||'',qty:Number(l.quantity)||0,unitPrice:Number(l.unitPriceSnapshot)||0,cost:Number(l.lineCost)||0,painter:(users||[]).find(x=>x.id===base?.mainPainterId)?.name||base?.mainPainterId||'—',supervisor:(users||[]).find(x=>x.id===base?.allottedSupervisorId)?.name||base?.allottedSupervisorId||'—'})})});
...
window.consPrintReport=function(){const x=document.getElementById('crResult'),w=window.open('','_blank');if(!x||!w)return;w.document.write('<html><head><title>Painting Consumables Report</title><style>body{font-family:Arial;padding:24px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #bbb;padding:7px;text-align:left}.cons-export-actions{display:none}</style></head><body><h2>Painting Consumables — Actual Materials Report</h2>'+x.innerHTML+'</body></html>');w.document.close();w.print()};
window.consShareReport=async function(){const x=document.getElementById('crResult');if(!x)return;const text='ZUKAIT AUTO — Painting Consumables Actual Materials Report\n'+x.innerText;if(navigator.share){try{await navigator.share({title:'Painting Consumables Report',text})}catch(e){}}else{try{await navigator.clipboard.writeText(text);alert('Report copied. You can paste it into WhatsApp or another app.')}catch(e){alert('Share is not available on this device.')}}};
