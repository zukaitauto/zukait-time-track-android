...
window.consOpenMaster=function(){if(role()!=='Manager')return;const units=['Liter','kg','Piece'];modal('Brands & Price','<div class="cons-subnav"><button class="secondary" type="button" onclick="openPaintingConsumables()">← BACK TO CONSUMABLES</button></div><div id="consMasterWorkspace" class="cons-entry-shell"><div class="cons-master-form"><input id="cmMaterial" placeholder="Material Name"><input id="cmBrand" placeholder="Brand Name"><select id="cmUnit">'+units.map(x=>'<option>'+x+'</option>').join('')+'</select><input id="cmPrice" type="number" min="0" step="0.001" placeholder="Price / Unit OMR"><label>Effective From<input id="cmDate" type="date" value="'+new Date().toISOString().slice(0,10)+'"></label><button class="blue" onclick="consSaveMaster()">ADD / SET PRICE</button></div><div id="cmList"></div><button class="secondary" onclick="consPrintMaster()">PRINT MATERIAL & PRICE LIST</button></div>');consRenderMaster()};
...
window.consRunPaintReport=function(){
 ...
 c.actuals.filter(x=>!x.voided&&x.locked&&(!from||day(x.actualAt)>=from)&&(!to||day(x.actualAt)<=to)&&(!jc||String(x.jobCard).toUpperCase().includes(jc))).forEach(a=>{
  ...
  a.lines.forEach(l=>{const m=c.materials.find(x=>x.id===l.materialId)||{},b=c.brands.find(x=>x.id===l.brandId)||{};if(String(m.unit||'').toLowerCase()!=='liter'||(bid&&l.brandId!==bid))return;
 ...
 const sum=k=>Math.round(rows.reduce((n,r)=>n+(Number(r[k])||0),0)*1000)/1000,totalCost=Math.round(rows.reduce((n,r)=>n+r.cost,0)*1000)/1000;
 out.innerHTML='<div class="cons-report-kpis paint"><div><b>'+sum('issued').toFixed(3)+' L</b><span>Total Paint Issued</span></div><div><b>'+sum('actual').toFixed(3)+' L</b><span>Actual Paint Used</span></div><div><b>'+sum('variance').toFixed(3)+' L</b><span>Variance</span></div><div><b>OMR '+totalCost.toFixed(3)+'</b><span>Total Paint Cost</span></div></div><div class="cons-paint-report-list">'+rows.map(r=>'<div class="cons-paint-card"><div class="head"><b>'+esc(r.jc)+'</b><span>'+esc(r.date)+'</span></div><strong>'+esc(r.vehicle||'Vehicle not recorded')+'</strong><small>Colour '+esc(r.colour||'—')+' · '+esc(r.painter)+' · '+esc(r.supervisor)+'</small><div class="metrics"><span>Issued<b>'+r.issued.toFixed(3)+' L</b></span><span>Additional<b>'+r.additional.toFixed(3)+' L</b></span><span>Actual<b>'+r.actual.toFixed(3)+' L</b></span><span>Variance<b>'+r.variance.toFixed(3)+' L</b></span></div><small>'+esc(r.material)+' · '+esc(r.brand)+' · OMR '+r.cost.toFixed(3)+'</small></div>').join('')+'</div><div class="cons-table cons-paint-wide"><table><tr><th>Date</th><th>JC</th><th>Vehicle</th><th>Colour</th><th>Painter</th><th>Supervisor</th><th>Material</th><th>Brand</th><th>Issued L</th><th>Additional L</th><th>Actual L</th><th>Variance L</th><th>Cost</th></tr>'+rows.map(r=>'<tr><td>'+esc(r.date)+'</td><td>'+esc(r.jc)+'</td><td>'+esc(r.vehicle)+'</td><td>'+esc(r.colour)+'</td><td>'+esc(r.painter)+'</td><td>'+esc(r.supervisor)+'</td><td>'+esc(r.material)+'</td><td>'+esc(r.brand)+'</td><td>'+r.issued.toFixed(3)+'</td><td>'+r.additional.toFixed(3)+'</td><td>'+r.actual.toFixed(3)+'</td><td>'+r.variance.toFixed(3)+'</td><td>OMR '+r.cost.toFixed(3)+'</td></tr>').join('')+'</table></div><div class="cons-export-actions"><button class="secondary" onclick="consPrintPaintReport()">PRINT / PDF</button><button class="blue" onclick="consSharePaintReport()">SHARE</button></div>';
};
window.consPrintPaintReport=function(){const x=document.getElementById('prResult');if(!x)return;const w=consPrintableWindow('Painting Consumables Paint Report','<h2>Painting Consumables — Paint Report</h2>'+x.innerHTML);if(!w)return alert('Allow pop-ups to print.');w.focus();w.print()};
window.consSharePaintReport=async function(){const x=document.getElementById('prResult');if(!x)return;const t='ZUKAIT AUTO — Painting Consumables Paint Report\n'+x.innerText;if(navigator.share){try{await navigator.share({title:'Paint Report',text:t})}catch(e){}}else{try{await navigator.clipboard.writeText(t);alert('Paint Report copied.')}catch(e){alert('Share is not available on this device.')}}};
window.consPrintReport=function(){const x=document.getElementById('crResult');if(!x)return;const w=consPrintableWindow('Painting Consumables Report','<h2>Painting Consumables — Actual Materials Report</h2>'+x.innerHTML);if(!w)return alert('Allow pop-ups to print.');w.focus();w.print()};
