...
window.v63OpenLeave=function(emp){
 ...
 '<label>Remark (optional)<br><input id="v63LeaveRemark" style="width:100%" placeholder=""></label><p><button class="green" onclick="v63SaveLeave(\''+esc(target)+'\')">MARK LEAVE</button></p>'+
...
window.v63OpenControl=function(type){const titles={today:'Today Jobs',working:'Working Now',notstarted:'Not Started',paused:'Work Paused'},rows=statusRows(type);showManagerModal(titles[type],'<div class="row"><input id="v63ControlSearch" style="flex:1" placeholder="" oninput="v63FilterControl()"></div><div id="v63ControlRows" data-type="'+type+'">'+controlTable(rows)+'</div>')}
