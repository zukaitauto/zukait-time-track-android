...
window.v84OpenDept=function(dept){let team=(users||[]).filter(u=>u.role==='Employee'&&u.department===dept),label=dept==='Denter'?'Denting':dept==='Painter'?'Painting':'Mechanical';let body='<div class="v84-tech-grid">'+(team.length?team.map(u=>{let x=v84TechState(u);return '<div class="v84-tech-card"><button class="v84-tech-name" onclick="v84ToggleTech(\''+u.id+'\',this)"><b>'+E(u.name)+'</b><span class="v84-status v84-status-'+E(x.status.toLowerCase())+'">● '+E(x.status)+'</span></button><div class="v84-tech-expand"></div></div>'}).join(''):'<div class="notice">No technicians in this department.</div>')+'</div>';showSupervisorModal('👷 '+E(label)+' Technicians',body)};
