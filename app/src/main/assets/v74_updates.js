...
(function(){'use strict';
 ...
 function apply(){
  ...
  root.querySelectorAll('.v118-action-grid,.v84-action-grid').forEach(x=>x.remove());let old=[...root.querySelectorAll('details.card')].find(x=>/Additional Action History/i.test(x.textContent||''));if(old)old.remove();let s=root.querySelector('#v119Actions');if(!s){s=document.createElement('section');s.id='v119Actions';s.className='card v119-actions';root.appendChild(s)}s.innerHTML='<div class="v119-actions-grid"><button onclick="openSupervisorJobCardList()">📋<b>Job Card List</b><small>View job card details</small></button><button onclick="v119OpenAssignedJobs()">🗂<b>Assigned Job Cards</b><small>View active assignments</small></button><button onclick="manualAdditionalTime()">⏱<b>Additional Time</b><small>Add allocated time</small></button></div><h3 class="v119-actions-title">Additional Action History</h3>';
