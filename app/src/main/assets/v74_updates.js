...
window.v132SaveSupervisorVehicleEdit=function(no){
 if(!me||me.role!=='Supervisor')return alert('Supervisor access required.');
 const j=(state.jobs||[]).find(x=>x&&String(x.no||'').trim().toUpperCase()===String(no||'').trim().toUpperCase());if(!j)return alert('Job Card not found.');
 const make=(document.getElementById('v132SupMake')?.value||'').trim(),model=(document.getElementById('v132SupModel')?.value||'').trim(),yearEl=document.getElementById('v132SupYear'),rawYear=String(yearEl?.value??'').trim(),year=rawYear,reg=(document.getElementById('v132SupReg')?.value||'').trim().toUpperCase();
 if(!make||!model||!reg)return alert('Make, Model and Registration Number are required.');if(rawYear&&!/^\\d{4}$/.test(rawYear))return alert('Enter a valid 4-digit vehicle year.');
