...
(function(){'use strict';
 ...
 window.v150OpenManagerSpareParts=()=>window.zukaitV2?.sparePartsMain?.open?window.zukaitV2.sparePartsMain.open():(typeof window.openSpareParts==='function'?window.openSpareParts():modal('Spare Parts','⚙️'));window.v153OpenManagerBodyShop=()=>modal('Body Shop','🚘');
 ...
 function apply(){if(!window.me||me.role!=='Manager')return;const root=document.getElementById('managerView');if(!root)return;const grid=root.querySelector('.v67-control-grid,.v66-control-grid,.v65-control-grid');if(!grid)return;
   ...
   if(!sp)sp=card('v154-spare');const cards=[...grid.children].filter(x=>x.nodeType===1&&x!==sp),fifthRowStart=8;if(cards.length>fifthRowStart)grid.insertBefore(sp,cards[fifthRowStart]);else grid.appendChild(sp);sp.style.gridColumn=''
