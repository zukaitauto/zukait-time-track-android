...
window.v132OpenSupervisorJobFull=function(no){
 ...
 const aa=(state.assign||[]).filter(a=>a&&!a.cancelled&&a.job===no),sessions=(state.sessions||[]).filter(s=>s&&s.job===no).sort((a,b)=>(a.start||0)-(b.start||0));
...
window.openSupervisorJobCardList=function(){
 ...
 if(typeof showSupervisorModal==='function')showSupervisorModal('📋 Job Card Details — All Job Cards',body);else if(typeof openModal==='function')openModal('<div class="section-title"><h2>📋 Job Card Details — All Job Cards</h2><button class="secondary" onclick="closeModal()">Close</button></div>'+body);setTimeout(v74JobListRender,0);clearInterval(window.v74JobListLiveTimer);window.v74JobListLiveTimer=setInterval(()=>{if(document.getElementById('v74jlrows'))v74JobListRender(true);else{clearInterval(window.v74JobListLiveTimer);window.v74JobListLiveTimer=null}},2000);
