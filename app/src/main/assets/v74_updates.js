...
function v75EffRows(key){let b=v75MonthBounds(key),map={},idx=window.zukaitDataIndex;(state.assign||[]).filter(a=>a&&!a.cancelled&&a.completed&&!a.rework&&a.job!==H&&+a.completedAt>=b.from&&+a.completedAt<b.to).forEach(a=>{let u=P(a.emp),r=map[a.emp]||(map[a.emp]={id:a.emp,name:u.name,department:u.department||'',jobs:0,suggested:0,actual:0});r.jobs++;r.suggested+=+a.suggested||0;(idx?idx.sessionsForJob(a.job):(state.sessions||[])).filter(s=>s&&(s.assignmentId===a.id||(!s.assignmentId&&s.job===a.job&&s.emp===a.emp))&&s.rework!==true).forEach(s=>{let st=Math.max(+s.start||0,b.from),en=Math.min(+(s.end||s.start)||0,b.to);if(en>st)r.actual+=v75NormalMinutes(st,en)})});return Object.values(map).map(r=>({...r,eff:r.actual>0?r.suggested/r.actual*100:null}))}
...
window.v92OpenAvailableWorkers=function(){let team=(users||[]).filter(u=>u.role==='Employee'&&!v84TechState(u).session),body=team.length?'<div class="v84-tech-grid v151-available-workers">'+team.map(u=>{const cc=typeof window.zukaitEmployeeColourClass==='function'?window.zukaitEmployeeColourClass(u.id):'';return '<div class="v84-tech-card v151-available-worker '+cc+'" data-emp-id="'+E(u.id)+'"><div class="v84-tech-name"><b class="v151-worker-name">'+E(u.name)+'</b><span class="v84-status v84-status-available">● Available</span></div><div class="small muted">'+E(u.department||'Technician')+'</div></div>'}).join('')+'</div>':'<div class="notice">No technicians are available now.</div>';showSupervisorModal('👷 Available Workers',body)};
...
window.v74JobListRows=function(){
 const q=(document.getElementById('v74jlq')?.value||'').trim().toLowerCase(),from=document.getElementById('v74jlf')?.value,to=document.getElementById('v74jlt')?.value,st=document.getElementById('v74jls')?.value||'all',emp=document.getElementById('v74jle')?.value||'all',dept=document.getElementById('v74jld')?.value||'all',sort=document.getElementById('v74jlo')?.value||'newest';
 ...
 if(q)jobs=jobs.filter(x=>[x.j.no,x.j.reg,x.j.vehicle,x.j.make,x.j.brand,x.j.model].join(' ').toLowerCase().includes(q));
 ...
 if(st==='finished'||st==='ready')jobs=jobs.filter(x=>x.finished); if(st==='paused')jobs=jobs.filter(x=>x.paused); if(st==='repeat')jobs=jobs.filter(x=>x.repeat); if(st==='working')jobs=jobs.filter(x=>x.aa.some(a=>{try{return !a.completed&&empStatus(a)==='Started'}catch(_){return false}})); if(st==='exceeded')jobs=jobs.filter(x=>x.aa.some(a=>{try{return !a.completed&&overStatus(a)}catch(_){return false}})); if(st==='notstarted')jobs=jobs.filter(x=>!x.finished&&!x.paused&&!x.repeat&&!x.aa.some(a=>{try{return !a.completed&&(empStatus(a)==='Started'||overStatus(a))}catch(_){return false}}));
 if(emp!=='all')jobs=jobs.filter(x=>x.aa.some(a=>String(a.emp)===emp));
 if(dept!=='all')jobs=jobs.filter(x=>x.aa.some(a=>String(v74JLP(a.emp).department||'').toLowerCase()===dept));
 jobs.sort(sort==='job'?((a,b)=>String(a.j.no).localeCompare(String(b.j.no),undefined,{numeric:true,sensitivity:'base'})):sort==='oldest'?((a,b)=>a.ts-b.ts):((a,b)=>b.ts-a.ts));
 return jobs;
};
window.v164JobListLimit=50;
window.v164ResetJobListLimit=function(){window.v164JobListLimit=50};
window.v164LoadMoreJobCards=function(){window.v164JobListLimit=(window.v164JobListLimit||50)+50;v74JobListRender(true)};
window.v74JobListRender=function(keepLimit){
 let el=document.getElementById('v74jlrows');if(!el)return;if(!keepLimit)window.v164JobListLimit=50;let rows=v74JobListRows();let sum=document.getElementById('v74jlsummary');if(sum){let working=rows.filter(x=>x.aa.some(a=>{try{return !a.completed&&empStatus(a)==='Started'}catch(_){return false}})).length,paused=rows.filter(x=>x.paused).length,finished=rows.filter(x=>x.finished).length;sum.innerHTML='<button onclick="v74SetJobListStatus(\'all\')"><b>Total '+rows.length+'</b></button><button onclick="v74SetJobListStatus(\'working\')">Working '+working+'</button><button onclick="v74SetJobListStatus(\'paused\')">Paused '+paused+'</button><button onclick="v74SetJobListStatus(\'ready\')">Ready '+finished+'</button>'}
 const statusOf=x=>{let working=x.aa.some(a=>{try{return !a.completed&&empStatus(a)==='Started'}catch(_){return false}}),over=x.aa.some(a=>{try{return !a.completed&&overStatus(a)}catch(_){return false}});return over?['EXCEEDED','exceeded']:working?['WORKING','working']:x.paused?['PAUSED','paused']:x.finished?['COMPLETED','completed']:x.repeat?['REPEAT WORK','repeat']:['NOT STARTED','new']};
...
window.v132OpenSupervisorJobFull=function(no){
 ...
 const working=aa.some(a=>{try{return !a.completed&&empStatus(a)==='Started'}catch(_){return false}}),paused=aa.some(a=>{try{return !a.completed&&empStatus(a)==='Paused'}catch(_){return false}}),finished=aa.length&&aa.every(a=>a.completed),over=aa.some(a=>{try{return !a.completed&&overStatus(a)}catch(_){return false}});
 const status=over?'EXCEEDED':working?'WORKING':paused?'PAUSED':finished?'COMPLETED':'NOT STARTED',cl=over?'exceeded':working?'working':paused?'paused':finished?'completed':'new';
 const tech=aa.length?aa.map(a=>{let actual=0;try{actual=totalForAssignment(a)}catch(_){ }let st='Not Started';try{st=empStatus(a)}catch(_){}return '<div class="v132-tech"><b>'+v74JLE(v74JLP(a.emp).name)+'</b><span>'+v74JLE(v74JLP(a.emp).department||'')+'</span><div>STATUS <strong>'+v74JLE(st)+'</strong></div><div>SUGGESTED <strong>'+fmt(a.suggested||0)+'</strong></div><div>ACTUAL <strong>'+fmt(actual)+'</strong></div></div>'}).join(''):'<div class="notice">No technician assigned.</div>';
 const hist=sessions.length?'<div class="v132-history">'+sessions.map(s=>'<div><b>'+v74JLE(v74JLP(s.emp).name)+'</b> · '+new Date(s.start).toLocaleString()+' → '+(s.end?new Date(s.end).toLocaleString():'Running')+'</div>').join('')+'</div>':'<div class="muted">No work sessions yet.</div>';
 const canReissue=finished&&aa.some(a=>!a.rework),reissue=canReissue?'<button class="purple" onclick="v132ReissueJob(\''+v74JLE(no)+'\')">REISSUE JOB</button>':'';
 const body='<div class="v132-full '+cl+'"><div class="v132-status">'+status+'</div><div class="v132-vehicle"><div><small>JOB CARD</small><h2>'+v74JLE(no)+'</h2></div><div><small>VEHICLE</small><h3>'+v74JLE(j.vehicle||'—')+(j.year?' · '+v74JLE(j.year):'')+'</h3><b>'+v74JLE(j.reg||'—')+'</b></div></div><div class="v132-summary"><div>Suggested<b>'+fmt(sg)+'</b></div><div>Actual<b>'+fmt(ac)+'</b></div><div>'+(over?'Exceeded':'Remaining')+'<b>'+fmt(over?Math.max(0,ac-sg):Math.max(0,sg-ac))+'</b></div></div><h3>Technicians</h3><div class="v132-tech-grid">'+tech+'</div><h3>Work History</h3>'+hist+'<div class="v132-actions"><button class="blue" onclick="v132OpenSupervisorVehicleEdit(\''+v74JLE(no)+'\')">EDIT VEHICLE DETAILS</button>'+reissue+'<button class="secondary" onclick="closeSupervisorModal()">BACK</button></div></div>';
 showSupervisorModal('Job Card Details',body);
};
window.v132ReissueJob=function(no){
 const aa=(state.assign||[]).filter(a=>a&&!a.cancelled&&a.job===no&&!a.rework&&a.completed);if(!aa.length)return alert('No completed assignment is available to reissue.');
 if(aa.length===1)return window.v71ReopenSameAssignment?window.v71ReopenSameAssignment(aa[0].id):alert('Reissue function is unavailable.');
 let opts=aa.map(a=>'<option value="'+v74JLE(a.id)+'">'+v74JLE(v74JLP(a.emp).name)+' · '+v74JLE(v74JLP(a.emp).department||'')+'</option>').join('');
 showSupervisorModal('Reissue Job Card '+no,'<div class="notice">Select the technician whose completed assignment should be reopened. Existing work history will be preserved.</div><label>Technician<br><select id="v132ReissueEmp">'+opts+'</select></label><p><button class="purple" onclick="v132ConfirmReissue(\''+v74JLE(no)+'\')">REISSUE</button></p>');
};
window.v132ConfirmReissue=function(no){let id=document.getElementById('v132ReissueEmp')?.value;if(!id)return;if(!confirm('Reissue Job Card '+no+' to the selected technician?'))return;if(window.v71ReopenSameAssignment)window.v71ReopenSameAssignment(id);else alert('Reissue function is unavailable.');};
if(!document.getElementById('v132-supervisor-jobcards')){let s=document.createElement('style');s.id='v132-supervisor-jobcards';s.textContent='.v164-load-more{display:flex;align-items:center;justify-content:center;gap:10px;padding:16px 6px;color:#64748b;font-size:11px;font-weight:800}.v164-load-more button{min-height:38px!important;margin:0!important}.v132-jc-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.v162-view-switch{display:flex;justify-content:flex-end;gap:7px;margin:0 0 10px}.v162-view-switch button{min-height:36px;padding:7px 12px;border-radius:10px;background:#eef2f7!important;color:#334155!important;box-shadow:none!important;border:1px solid #cbd5e1!important;font-size:10px!important}.v162-view-switch button.active{background:#17324b!important;color:#fff!important;border-color:#17324b!important}.v132-jc-grid.v162-list-view{display:block}.v132-jc-grid.v162-list-view .v132-jc-card{display:grid;grid-template-columns:90px minmax(120px,1.4fr) minmax(90px,1fr) minmax(100px,1fr);align-items:center;gap:8px;min-height:62px;margin-bottom:8px;padding:9px 11px;border-radius:12px;box-shadow:0 3px 0 #8aafd4,0 6px 12px #17324b18}.v132-jc-grid.v162-list-view .v132-jc-top{display:block}.v132-jc-grid.v162-list-view .v132-jc-top span{display:block;width:max-content;margin:4px 0 0}.v132-jc-grid.v162-list-view .v132-jc-card h3{margin:0;font-size:12px}.v132-jc-grid.v162-list-view .v132-jc-card p{margin:2px 0 0;font-weight:900;font-size:11px}.v132-jc-grid.v162-list-view .v132-jc-tech{font-size:10px}.v132-jc-grid.v162-list-view .v132-jc-mini{margin:0;gap:4px}.v132-jc-grid.v162-list-view .v132-jc-mini div{padding:5px;font-size:8px}.v132-jc-grid.v162-list-view .v132-jc-mini b{font-size:10px}.v132-jc-card{position:relative;text-align:left;color:#172033;border:1px solid #9bbfe5;border-radius:20px;padding:13px;background:linear-gradient(145deg,#eaf4ff,#d9eaff 58%,#c8def8);box-shadow:0 7px 0 #8aafd4,0 12px 20px #17324b25,inset 0 2px #fff;overflow:hidden}.v132-jc-card.working{background:linear-gradient(145deg,#e8fff0,#cff3dc,#b9e6ca);border-color:#6bbd8a;box-shadow:0 7px 0 #71ae88,0 12px 20px #17653b25}.v132-jc-card.paused{background:linear-gradient(145deg,#fff7d8,#f8e5a3,#e9cb6c);border-color:#d4ad38;box-shadow:0 7px 0 #b58c22,0 12px 20px #6b531d25}.v132-jc-card.exceeded{background:linear-gradient(145deg,#fff0f0,#ffd6d6,#f2baba);border-color:#df8585;box-shadow:0 7px 0 #bd6b6b,0 12px 20px #711f1f25}.v132-jc-card.completed{background:linear-gradient(145deg,#f1f5f9,#e2e8f0,#cbd5e1);border-color:#94a3b8;box-shadow:0 7px 0 #94a3b8,0 12px 20px #33415520}.v132-jc-top{display:flex;align-items:center;gap:8px}.v132-jc-top b{font-size:14px}.v132-jc-top span{margin-left:auto;font-size:9px;font-weight:1000;border-radius:12px;padding:5px 7px;background:#ffffffa8}.v132-jc-card h3{margin:10px 0 3px}.v132-jc-card p{margin:0 0 8px}.v132-jc-tech{font-size:11px;font-weight:800}.v132-jc-mini,.v132-summary{display:grid;grid-template-columns:repeat(2,1fr);gap:7px;margin-top:10px}.v132-jc-mini div,.v132-summary div{background:#ffffff9c;border-radius:11px;padding:8px;text-align:center;font-size:9px}.v132-jc-mini b,.v132-summary b{display:block;font-size:14px;margin-top:3px}.v132-full{padding-bottom:12px}.v132-status{text-align:center;font-size:14px;font-weight:1000;padding:9px;border-radius:14px;background:#dbeafe}.v132-full.working .v132-status{background:#d1fae5}.v132-full.paused .v132-status{background:#fef3c7}.v132-full.exceeded .v132-status{background:#fee2e2}.v132-vehicle{display:grid;grid-template-columns:.7fr 1.3fr;gap:10px;margin:12px 0}.v132-vehicle>div,.v132-tech{padding:12px;border:1px solid #dbe3ee;border-radius:15px;background:#f8fafc}.v132-vehicle h2,.v132-vehicle h3{margin:4px 0}.v132-summary{grid-template-columns:repeat(3,1fr)}.v132-tech-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.v132-tech span{display:block;font-size:10px;margin-bottom:8px}.v132-tech div{font-size:9px;margin-top:4px}.v132-tech strong{float:right;font-size:11px}.v132-history div{padding:8px 3px;border-bottom:1px solid #e2e8f0;font-size:11px}.v132-actions{position:sticky;bottom:0;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;padding:12px 0 2px;background:linear-gradient(180deg,#ffffff00,#fff 24%);z-index:6}.v132-actions button{min-height:48px}.v132-full{min-width:0}.v132-full *{box-sizing:border-box}.v132-full h2,.v132-full h3,.v132-jc-card h3,.v132-jc-card p{overflow-wrap:anywhere}.v132-jc-card{width:100%;min-width:0;min-height:150px}.v132-jc-grid,.v132-tech-grid,.v132-summary,.v132-vehicle{min-width:0}.v132-history{max-height:240px;overflow:auto}.v132-full .purple{background:#7c3aed!important;color:#fff!important}@media(max-width:480px){.v132-jc-grid,.v132-tech-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.v132-jc-card{padding:10px;min-height:142px}.v132-vehicle{grid-template-columns:1fr}.v132-summary{grid-template-columns:repeat(3,minmax(0,1fr))}.v132-actions{grid-template-columns:repeat(2,minmax(0,1fr))}}';document.head.appendChild(s)}
window.v164JobListObserver=null;
window.v164ObserveJobListMore=function(){if(window.v164JobListObserver)window.v164JobListObserver.disconnect();const b=document.querySelector('.v164-load-more button');if(!b||!('IntersectionObserver' in window))return;window.v164JobListObserver=new IntersectionObserver(es=>{if(es.some(e=>e.isIntersecting)){window.v164JobListObserver.disconnect();window.v164LoadMoreJobCards()}},{root:null,rootMargin:'300px'});window.v164JobListObserver.observe(b)};
window.v162SetJobListView=function(view){localStorage.setItem('zukaitJobListView',view==='list'?'list':'cards');document.querySelectorAll('.v162-view-switch button').forEach(b=>b.classList.toggle('active',b.dataset.view===(view==='list'?'list':'cards')));v74JobListRender()};
window.v74SetJobListStatus=function(st){let el=document.getElementById('v74jls');if(el)el.value=st;v74JobListRender()};
window.v74ClearJobListFilters=function(){['v74jlf','v74jlt','v74jlq'].forEach(id=>{let e=document.getElementById(id);if(e)e.value=''});[['v74jls','all'],['v74jle','all'],['v74jld','all'],['v74jlo','newest']].forEach(([id,v])=>{let e=document.getElementById(id);if(e)e.value=v});v74JobListRender()};
...
window.v132OpenSupervisorVehicleEdit=function(no){
 ...
 const body='<div class="notice"><b>Job Card:</b> '+v74JLE(j.no)+'</div><div class="grid"><label>Make<br><input id="v132SupMake" value="'+v74JLE(guessedMake)+'"></label><label>Model<br><input id="v132SupModel" value="'+v74JLE(guessedModel)+'"></label><label>Year<br>'+'<input id="v132SupYear" inputmode="numeric" maxlength="4" value="'+v74JLE(j.year||'')+'" placeholder="4-digit year">'+'</label><label>Registration Number<br><input id="v132SupReg" value="'+v74JLE(j.reg||'')+'"></label></div><div class="notice small">Only vehicle details are changed. Job Card number, technicians and work-time history remain unchanged.</div><p><button class="green" onclick="v132SaveSupervisorVehicleEdit(\''+v74JLE(j.no)+'\')">SAVE CHANGES</button></p>';
...
window.v132SaveSupervisorVehicleEdit=function(no){
 ...
 j.make=make;j.brand=make;j.model=model;j.year=year;j.reg=reg;j.vehicle=after.vehicle;
 state.jobVehicleEdits=state.jobVehicleEdits||[];state.jobVehicleEdits.push({job:j.no,before,after,by:me.id,byName:me.name||me.id,at:typeof now==='function'?now():Date.now()});
...
(function(){'use strict';
 ...
 if(typeof oldMonthlySuggested==='function')window.monthlySuggestedMinutes=function(emp,from,to){
   // Confirmed workshop rule: ID001 allocated/suggested time counts in monthly Suggested Time.
   // It remains excluded from productive Actual, Efficiency, Incentive, Overtime and Labour Cost.
   return (state.assign||[]).filter(a=>a&&a.emp===emp&&!a.cancelled&&(a.assignedAt||0)>=from&&(a.assignedAt||0)<to).reduce((n,a)=>n+(+a.suggested||0),0);
 };
 ...
 window.renderEmployee=function(){
   ...
   const vehicleBrand=v=>{const s=String(v||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[._/]+/g,' ').replace(/\s+/g,' ').trim();const mainBrands=[['Mercedes-Benz','mercedes-benz.svg',/\b(?:mercedes(?:[ -]?benz)?|benz)\b/],['BMW','bmw.svg',/\bbmw\b/],['Land Rover','land-rover.svg',/\b(?:land[ -]?rover|range[ -]?rover)\b/],['Lexus','lexus.svg',/\blexus\b/],['Jaguar','jaguar.svg',/\bjaguar\b/],['Toyota','toyota.svg',/\btoyota\b/],['Nissan','nissan-logo.svg',/\bnissan\b/],['Infiniti','infiniti-logo.svg',/\binfiniti\b/],['Porsche','porsche.svg',/\bporsche\b/],['Audi','audi.svg',/\baudi\b/],['Volkswagen','volkswagen.svg',/\b(?:volkswagen|vw)\b/],['Volvo','volvo.svg',/\bvolvo\b/],['Mitsubishi','mitsubishi.svg',/\bmitsubishi\b/],['Mazda','mazda.svg',/\bmazda\b/],['Jeep','jeep.svg',/\bjeep\b/],['Hyundai','hyundai-logo.svg',/\bhyundai\b/],['Kia','kia-logo.svg',/\bkia\b/],['Genesis','genesis-logo.svg',/\bgenesis\b/],['BYD','byd-logo.svg',/\bbyd\b/],['Jetour','jetour.svg',/\bjetour\b/],['Geely','geely.svg',/\bgeely\b/],['Lincoln','lincoln.svg',/\blincoln\b/],['Chrysler','chrysler.svg',/\bchrysler\b/],['Isuzu','isuzu.svg',/\bisuzu\b/],['RAM','ram.svg',/\bram\b/],['Suzuki','suzuki.svg',/\bsuzuki\b/],['Renault','renault.svg',/\brenault\b/],['Peugeot','peugeot.svg',/\bpeugeot\b/],['Citroën','citroen.svg',/\bcitroen\b/],['Škoda','skoda.svg',/\bskoda\b/],['Bentley','bentley.svg',/\bbentley\b/],['Rolls-Royce','rolls-royce.svg',/\brolls[ -]?royce\b/],['Aston Martin','aston-martin.svg',/\baston[ -]?martin\b/],['Ferrari','ferrari.svg',/\bferrari\b/],['McLaren','mclaren.svg',/\bmclaren\b/],['MAN','man.svg',/\bman\b/],['GWM','gwm.svg',/\b(?:great[ -]?wall|gwm)\b/],['Shacman','shacman.svg',/\b(?:shacman|shaanxi)\b/]];for(const b of mainBrands)if(b[2].test(s))return{name:b[0],file:b[1]};const brands=[
...
.v75s-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin:12px 10px}.v75s-card{position:relative;background:linear-gradient(145deg,#eaf4ff,#d9eaff 58%,#c8def8);border:1px solid #9bbfe5;border-radius:20px;padding:12px;box-shadow:0 8px 0 #8aafd4,0 13px 22px #17324b2b,inset 0 2px 2px #ffffffd9;transform:translateY(0);overflow:hidden}.v75s-card:before{content:"";position:absolute;inset:0 0 auto 0;height:42%;background:linear-gradient(180deg,#ffffff8a,#ffffff0a);pointer-events:none}.v75s-card>*{position:relative;z-index:1}.v75s-card.running{border-color:#6bbd8a;background:linear-gradient(145deg,#e8fff0,#cff3dc 58%,#b9e6ca);box-shadow:0 8px 0 #71ae88,0 13px 22px #17653b25,inset 0 2px 2px #fff}.v75s-card.ideal{border-color:#d3ae67;background:linear-gradient(145deg,#fff7df,#f6e5b9 58%,#ead39a);box-shadow:0 8px 0 #c4a15e,0 13px 22px #6f531d25,inset 0 2px 2px #fff}.v75s-card-top{display:flex;gap:6px;align-items:center}.v75s-card-top b{font-size:13px}.v75s-badge{margin-left:auto;border-radius:12px;padding:4px 7px;font-size:8px;font-weight:1000;background:#e9eef3;color:#36495b}.v75s-card.running .v75s-badge{background:#dbf8df;color:#166534}.v75s-card h3{font-size:15px;margin:10px 0 4px}.v75s-card p{font-size:11px;color:#5b6e7c;margin:0 0 9px}.v75s-mini{display:grid;grid-template-columns:1fr 1fr;gap:6px}.v75s-mini div{background:#ffffff9c;border:1px solid #ffffffc9;border-radius:11px;padding:7px;text-align:center;font-size:9px;box-shadow:inset 0 1px #fff,0 2px 5px #17324b14}.v75s-mini b{display:block;font-size:13px;margin-top:3px}.v75s-start{width:100%;margin-top:9px;border-radius:11px;padding:10px;font-weight:1000;background:linear-gradient(#378bf5,#1d6fd4);box-shadow:0 4px 0 #1154a5}.v75s-start[disabled]{background:#d8e1e8;color:#64748b;box-shadow:none}.v75s-parts{width:100%;margin-top:9px;border-radius:11px;padding:9px;font-size:11px;font-weight:1000;background:linear-gradient(#e0f2fe,#bae6fd)!important;color:#075985!important;border:1px solid #7dd3fc!important;box-shadow:0 3px 0 #38bdf8!important}.v75s-parts.empty{background:#eef2f6!important;color:#7b8794!important;border-color:#d5dde5!important;box-shadow:none!important}.v75s-card .v75s-car{transform:scale(.62);transform-origin:left center;margin:5px 0 7px}.v75s-card .v82-vehicle-mark{height:38px;min-height:38px;margin:4px 0 8px;align-items:center;overflow:hidden}.v75s-card .v82-brand-logo{width:52px;height:32px;min-width:52px;max-width:52px;padding:4px;overflow:hidden}.v75s-card .v82-brand-logo img{display:block;width:100%!important;height:100%!important;max-width:100%!important;max-height:100%!important;object-fit:contain!important}.v82-brand-logo[title="Kia"] img{transform:scale(1.18);transform-origin:center}.v82-brand-logo[title="Mercedes-Benz"] img{width:88%!important;height:88%!important;margin:auto!important;object-position:center center!important;transform:none!important}.v75s-card .v82-logo-fallback{height:32px;display:flex;align-items:center;overflow:hidden}.v75s-card .v80-generic-car{font-size:30px;line-height:1}
...
/* Distinct updater state colours: check=blue, available/download=orange, progress=purple, install=green, success=green, failure=red. */
(function(){
 const paint=(el,cls)=>{if(!el)return;el.classList.remove('v77-check-state','v77-download-state','v77-progress-state','v77-install-state','v77-error-state');if(cls)el.classList.add(cls)};
 const oldCheck=window.v77CheckUpdate;window.v77CheckUpdate=function(){paint(document.getElementById('v77CheckBtn'),'v77-check-state');return oldCheck&&oldCheck.apply(this,arguments)};
 const oldResult=window.v72UpdateCheckResult;window.v72UpdateCheckResult=function(code,name,error){const r=oldResult&&oldResult.apply(this,arguments),down=document.getElementById('v77DownloadBtn'),check=document.getElementById('v77CheckBtn');paint(check,error?'v77-error-state':'v77-check-state');if(down&&Number(code)>currentCode())paint(down,'v77-download-state');return r};
 const oldStatus=window.v77UpdateDownloadStatus;window.v77UpdateDownloadStatus=function(status){const r=oldStatus&&oldStatus.apply(this,arguments),down=document.getElementById('v77DownloadBtn'),inst=document.getElementById('v77InstallBtn'),bar=document.getElementById('v77ProgressBar');if(status==='PENDING'||status==='DOWNLOADING'||status==='PAUSED'){paint(down,'v77-progress-state');paint(bar,'v77-progress-state')}else if(status==='COMPLETE'||status==='INSTALLING'){paint(inst,'v77-install-state');paint(bar,'v77-install-state')}else if(status==='FAILED'||status==='INSTALL_DIAGNOSTIC'){paint(down,'v77-error-state');paint(inst,'v77-error-state');paint(bar,'v77-error-state')}return r};
 const s=document.createElement('style');s.id='v77UpdateStateColours';s.textContent='.v77-check-state{background:#2563eb!important;color:#fff!important;border-color:#1d4ed8!important}.v77-download-state{background:#f59e0b!important;color:#422006!important;border-color:#d97706!important}.v77-progress-state{background:#7c3aed!important;color:#fff!important;border-color:#6d28d9!important}.v77-install-state{background:#16a34a!important;color:#fff!important;border-color:#15803d!important}.v77-error-state{background:#dc2626!important;color:#fff!important;border-color:#b91c1c!important}#v77ProgressBar.v77-progress-state{background:#7c3aed!important}#v77ProgressBar.v77-install-state{background:#16a34a!important}#v77ProgressBar.v77-error-state{background:#dc2626!important}';document.head.appendChild(s);
})();
...
(function(){'use strict';
 function openManagerIncentive(){
   ...
   const body='<div class="section-title"><h2>⭐ Monthly Incentive</h2><button class="secondary" onclick="closeModal()">Close</button></div>'+
    '<p class="muted">Uses the same final monthly calculation as Employee and Supervisor. ID001 duty-time is included in Achieved but excluded from productive efficiency and JC labour cost; overtime is excluded.</p>'+
...
/* V154 MANAGER WORKSHOP CONTROL ROOT AUTHORITY — fifth-row Spare Parts entry, deduplicated. */
(function(){'use strict';
 ...
 function apply(){if(!window.me||me.role!=='Manager')return;const root=document.getElementById('managerView');if(!root)return;const grid=root.querySelector('.v67-control-grid,.v66-control-grid,.v65-control-grid');if(!grid)return;
   ...
   if(!sp){sp=card('v154-spare');grid.appendChild(sp)}
...
(function(){'use strict';
 ...
 function bounds(){const today=!!document.querySelector('#managerView .v123-toggle button:first-child.on'),d=new Date();if(today){const f=new Date(d);f.setHours(0,0,0,0);return{from:+f,to:+f+86400000,label:'Today'}}return{from:+new Date(d.getFullYear(),d.getMonth(),1),to:+new Date(d.getFullYear(),d.getMonth()+1,1),label:'This Month'}}
 function calc(from,to){const api=window.ZukaitConsumables;if(!api)return{total:0,paint:0,other:0,jobs:0,rows:[],top:null};let c;try{c=api.ensureState(state)}catch(_){return{total:0,paint:0,other:0,jobs:0,rows:[],top:null}}const rows=[];(c.actuals||[]).filter(a=>a&&!a.voided&&a.locked&&(+a.actualAt||0)>=from&&(+a.actualAt||0)<to).forEach(a=>(a.lines||[]).forEach(l=>{const m=(c.materials||[]).find(x=>x.id===l.materialId)||{},b=(c.brands||[]).find(x=>x.id===l.brandId)||{},cost=Number(l.lineCost)||0;rows.push({jc:a.jobCard||'',material:m.name||'',brand:b.name||'',unit:m.unit||'',category:m.category||'Consumable',cost})}));let paint=0,other=0;rows.forEach(x=>{if(String(x.category).toLowerCase()==='paint')paint+=x.cost;else other+=x.cost});const by={};rows.forEach(x=>by[x.jc]=(by[x.jc]||0)+x.cost);const top=Object.entries(by).sort((a,b)=>b[1]-a[1])[0]||null;return{total:paint+other,paint,other,jobs:Object.keys(by).length,rows,top}}
 window.v145OpenConsumablesPerformance=function(){if(!me||me.role!=='Manager')return;const b=bounds(),x=calc(b.from,b.to),top=x.top?'<div class="notice"><b>Highest Cost Job Card</b><div>'+escx(x.top[0])+' · OMR '+x.top[1].toFixed(3)+'</div></div>':'';const body='<div class="v145-cons-break"><div><span>Paint Expense</span><b>OMR '+x.paint.toFixed(3)+'</b></div><div><span>Other Consumables</span><b>OMR '+x.other.toFixed(3)+'</b></div><div><span>Total Expense</span><b>OMR '+x.total.toFixed(3)+'</b></div><div><span>Job Cards</span><b>'+x.jobs+'</b></div></div>'+top+'<p><button class="blue" onclick="closeModal();setTimeout(function(){if(typeof consOpenReports===\'function\')consOpenReports()},0)">VIEW FULL REPORT</button></p>';if(typeof openModal==='function')openModal('<div class="section-title"><div><h2>Consumables Expense</h2><small>'+escx(b.label)+' · finalized Actual Materials</small></div><button class="secondary" onclick="closeModal()">Close</button></div>'+body)};
 function apply(){if(!me||me.role!=='Manager')return;const grid=document.querySelector('#managerView .v123-manager-performance .v123-grid');if(!grid)return;let btn=grid.querySelector('.v145-consumables');if(!btn){btn=document.createElement('button');btn.className='v123-perf v145-consumables';btn.onclick=window.v145OpenConsumablesPerformance;grid.appendChild(btn)}const b=bounds(),x=calc(b.from,b.to);btn.innerHTML='<span>Consumables Expense</span><b>'+x.total.toFixed(3)+' OMR</b><small>'+b.label+' · '+x.jobs+' job card'+(x.jobs===1?'':'s')+'</small>'}
 const oldRange=window.v123ManagerPerformanceRange;if(typeof oldRange==='function')window.v123ManagerPerformanceRange=function(v){const r=oldRange.apply(this,arguments);setTimeout(apply,0);return r};
 const prev=window.render;window.render=function(){const r=typeof prev==='function'?prev.apply(this,arguments):undefined;setTimeout(apply,0);return r};window.v145ApplyConsumablesPerformance=apply;
 const s=document.createElement('style');s.id='v145ConsumablesPerformanceStyle';s.textContent='.v145-consumables small{display:block;margin-top:4px;font-size:9px;color:#667085}.v145-cons-break{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:10px 0}.v145-cons-break>div{padding:12px;border:1px solid #dce5ee;border-radius:12px;background:#f8fafc}.v145-cons-break span,.v145-cons-break b{display:block}.v145-cons-break span{font-size:10px;font-weight:800;color:#667085}.v145-cons-break b{font-size:16px;margin-top:4px;color:#172033}';document.head.appendChild(s);
 setTimeout(apply,0);
})();
...
/* V2 CONTROLLED PILOT — Manager may opt this device into V2 authority; all other devices remain legacy. */
window.v2TogglePilotThisDevice=function(){
 if(!window.me||me.role!=='Manager')return alert('Manager access required.');
 const p=window.zukaitV2?.pilot,a=window.zukaitV2?.authorityAdapter;
 if(!p||!a)return alert('V2 pilot controls are unavailable in this build.');
 if(a.enabled()){
   p.disable();
   return alert('V2 pilot disabled on this device. Legacy authority is active.');
 }
 const id=p.deviceId();
 if(!id)return alert('Device ID is unavailable. Synchronize once and try again.');
 p.setPilotDevice(id);
 const r=p.enable();
 alert(r.ok?'V2 pilot enabled on this device only.':'Unable to enable V2 pilot: '+String(r.reason||'unknown'));
};
...
(function(){'use strict';
 ...
 function report(no){
   const j=typeof job==='function'?job(no):null;if(!j)return'';
   const rows=(state.assign||[]).filter(a=>a&&a.job===no&&!a.cancelled);
   const rate=Number(state.labourRate||2.5);
   const suggested=rows.reduce((n,a)=>n+(+a.suggested||0),0);
   const actual=rows.reduce((n,a)=>n+(typeof totalForAssignment==='function'?(+totalForAssignment(a)||0):0),0);
   const assignedDate=typeof jobAssignedDate==='function'?jobAssignedDate(j):j.createdAt;
   const status=typeof jobManagerStatus==='function'?jobManagerStatus(j):(j.status||'—');
   const team=rows.map(a=>{const u=typeof user==='function'?user(a.emp):{name:a.emp,department:''};const ac=typeof totalForAssignment==='function'?(+totalForAssignment(a)||0):0;const sg=+a.suggested||0;return '<tr><td>'+esc(u.name||a.emp)+'</td><td>'+esc(u.department||'—')+'</td><td>'+fmt(sg)+'</td><td>'+fmt(ac)+'</td><td>OMR '+(sg/60*rate).toFixed(3)+'</td><td>OMR '+(ac/60*rate).toFixed(3)+'</td></tr>'}).join('');
   const sessions=(state.sessions||[]).filter(s=>s&&s.job===no).sort((a,b)=>(+a.start||0)-(+b.start||0)).map(s=>{const u=typeof user==='function'?user(s.emp):{name:s.emp};const end=s.end||Date.now();return '<tr><td>'+esc(u.name||s.emp)+'</td><td>'+new Date(s.start).toLocaleString()+'</td><td>'+(s.end?new Date(s.end).toLocaleString():'Working now')+'</td><td>'+fmt((end-s.start)/60000)+'</td></tr>'}).join('');
   return '<!doctype html><html><head><meta charset="utf-8"><style>@page{size:A4;margin:12mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#172033;font-size:11px}h1{font-size:20px;margin:0}h2{font-size:14px;margin:18px 0 7px}.head{border-bottom:2px solid #172033;padding-bottom:10px;margin-bottom:12px}.sub{color:#5b6575;margin-top:4px}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}.box{border:1px solid #d6dce5;border-radius:7px;padding:8px}.box b{display:block;font-size:9px;color:#687386;margin-bottom:4px}.box strong{font-size:13px}table{width:100%;border-collapse:collapse;margin-top:6px}th,td{border:1px solid #d6dce5;padding:6px;text-align:left}th{background:#f1f4f8;font-size:9px}.foot{margin-top:18px;padding-top:8px;border-top:1px solid #d6dce5;color:#687386;font-size:9px}</style></head><body><div class="head"><h1>ZUKAIT INTERNATIONAL LLC</h1><div class="sub">Manager Job Card Review</div></div><div class="grid"><div class="box"><b>JOB CARD</b><strong>'+esc(j.no)+'</strong></div><div class="box"><b>VEHICLE / MODEL</b><strong>'+esc(j.vehicle||'—')+'</strong></div><div class="box"><b>REGISTRATION</b><strong>'+esc(j.reg||'—')+'</strong></div><div class="box"><b>ASSIGNED / CREATED</b><strong>'+(assignedDate?new Date(assignedDate).toLocaleString():'—')+'</strong></div><div class="box"><b>STATUS</b><strong>'+esc(status)+'</strong></div><div class="box"><b>LABOUR RATE</b><strong>OMR '+rate.toFixed(3)+'/hr</strong></div><div class="box"><b>TOTAL SUGGESTED</b><strong>'+fmt(suggested)+'</strong></div><div class="box"><b>TOTAL ACTUAL</b><strong>'+fmt(actual)+'</strong></div><div class="box"><b>TOTAL ACTUAL LABOUR COST</b><strong>OMR '+(actual/60*rate).toFixed(3)+'</strong></div></div><h2>Technician Labour Review</h2><table><tr><th>Technician</th><th>Department</th><th>Suggested</th><th>Actual</th><th>Suggested Cost</th><th>Actual Cost</th></tr>'+(team||'<tr><td colspan="6">No technician assignments.</td></tr>')+'</table><h2>Work Session History</h2><table><tr><th>Technician</th><th>Start</th><th>End</th><th>Duration</th></tr>'+(sessions||'<tr><td colspan="4">No work sessions recorded.</td></tr>')+'</table><div class="foot">Generated from Zukait Time Track · '+new Date().toLocaleString()+'</div></body></html>';
 }
 window.v140PrintManagerJob=function(no){const html=report(no);if(!html)return alert('Job Card not found.');if(window.AndroidBridge&&typeof AndroidBridge.printHtml==='function')AndroidBridge.printHtml(html);else{const w=window.open('','_blank');if(w){w.document.write(html);w.document.close();w.print()}else alert('Printing is not available on this device.')}};
 window.v140PdfManagerJob=function(no){const html=report(no);if(!html)return alert('Job Card not found.');const fn='Zukait_Job_Card_'+String(no).replace(/[^A-Za-z0-9_-]/g,'_')+'.pdf';if(window.AndroidBridge&&typeof AndroidBridge.shareHtmlAsPdf==='function')AndroidBridge.shareHtmlAsPdf(html,fn);else{const w=window.open('','_blank');if(w){w.document.write(html);w.document.close();w.print()}else alert('PDF export is not available on this device.')}};
 const old=window.openManagerJobDetails;
...
/* V134 SUPERVISOR QUICK ENTRY VOICE ASSIST */
(function(){'use strict';
 function box(){return document.getElementById('v143VoiceStatus')}
 window.v143StartVoiceEntry=function(){const b=box();if(b){b.textContent='Listening... Say job card, vehicle, year, registration, technician and time.';b.classList.remove('hidden')}if(window.AndroidBridge&&typeof AndroidBridge.startNativeVoiceRecognition==='function')AndroidBridge.startNativeVoiceRecognition();else if(b)b.textContent='Voice entry is available in the Android app.'};
...
/* V152 VEHICLE YEAR AUTHORITY — one normalizer/validator for create + edit flows. */
(function(){'use strict';
 window.zukaitVehicleYear=function(v){
   const raw=String(v??'').trim();
   if(!raw)return {ok:true,value:''};
   const digits=raw.replace(/[^0-9]/g,'');
   if(digits.length!==4)return {ok:false,value:raw};
   const n=Number(digits),max=(new Date()).getFullYear()+1;
   if(!Number.isInteger(n)||n<1900||n>max)return {ok:false,value:digits};
   return {ok:true,value:digits};
 };
 window.zukaitReadVehicleYear=function(id){const e=document.getElementById(id),r=window.zukaitVehicleYear(e?.value);if(r.ok&&e)e.value=r.value;return r};
})();
