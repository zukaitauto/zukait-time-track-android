...
function v84TechnicianBoard(){let ds=[['Denter','DENTER','🛠️','v84-denter'],['Painter','PAINTER','🎨','v84-painter'],['Mechanic','MECHANIC','⚙️','v84-mechanic']];return '<div class="card v84-tech-board"><h3>👷 Technician Board</h3><div class="v84-depts">'+ds.map(d=>{let t=(users||[]).filter(u=>u.role==='Employee'&&u.department===d[0]);return '<button class="v84-dept '+d[3]+'" onclick="v84OpenDept(\''+d[0]+'\')"><span>'+d[2]+'</span><b>'+d[1]+'</b><strong>'+t.length+'</strong></button>'}).join('')+'</div></div>'}
window.v84OpenDept=function(dept){let host=document.getElementById('v84TechDetails');if(!host)return;let team=(users||[]).filter(u=>u.role==='Employee'&&u.department===dept);host.innerHTML='<div class="card v84-tech-details"><div class="section-title"><h3>'+E(dept)+' Technicians</h3><button class="secondary" onclick="document.getElementById(\'v84TechDetails\').innerHTML=\'\'">Close</button></div><div class="v84-tech-grid">'+(team.length?team.map(u=>{let s=activeSession(u.id),st=s?(s.paused?'Paused':'Working'):'Available';return '<div class="v84-tech-card"><button class="v84-tech-name" onclick="v84ToggleTech(\''+u.id+'\',this)"><b>'+E(u.name)+'</b><span>'+E(st)+'</span></button><div class="v84-tech-expand"></div></div>'}).join(''):'<div class="notice">No technicians in this department.</div>')+'</div></div>';host.scrollIntoView({behavior:'smooth',block:'nearest'})};
window.v84ToggleTech=function(emp,btn){let root=btn.closest('.v84-tech-details');root.querySelectorAll('.v84-tech-expand').forEach(x=>{if(x!==btn.nextElementSibling)x.innerHTML=''});let out=btn.nextElementSibling;if(out.innerHTML){out.innerHTML='';return}let rows=(state.assign||[]).filter(a=>a.emp===emp&&!a.cancelled&&!a.completed);let a=rows.find(x=>{try{return empStatus(x)==='Started'}catch(_){return false}})||rows.find(x=>{try{return empStatus(x)==='Paused'}catch(_){return false}})||rows[0];if(!a){out.innerHTML='<div class="v84-tech-body"><b>AVAILABLE</b><br>No active job card.</div>';return}let j=J(a.job)||{},ac=A(a),sg=+a.suggested||0,rm=Math.max(0,sg-ac),ex=Math.max(0,ac-sg),st='Assigned';try{st=empStatus(a)}catch(_){}out.innerHTML='<div class="v84-tech-body"><b>JC '+E(a.job)+'</b><br>'+E(j.vehicle||'')+' · '+E(j.reg||'')+'<br>Status: <b>'+E(st)+'</b><br>Suggested: <b>'+F(sg)+'</b> · Actual: <b>'+F(ac)+'</b><br>'+(ex>0?'Exceeded: <b>'+F(ex)+'</b>':'Remaining: <b>'+F(rm)+'</b>')+'</div>'};
...
function v84SupervisorPanels(root){
 let req=[...root.querySelectorAll('.supervisor-request-top,.card')].find(x=>/EMPLOYEE REQUESTS/i.test(x.textContent||'')),att=[...root.querySelectorAll('.card')].find(x=>/Need[s]? Attention/i.test(x.textContent||''));if(req&&att&&!root.querySelector('.v84-alert-row')){let row=document.createElement('div');row.className='v84-alert-row';req.parentNode.insertBefore(row,req);row.append(req,att);req.classList.add('v84-alert','v84-request');att.classList.add('v84-alert','v84-attention');}
 [...root.querySelectorAll('.overtime-card')].forEach(x=>x.remove());
 let find=t=>[...root.querySelectorAll('.card')].find(x=>(x.textContent||'').includes(t)),job=find('Job Card List'),assigned=find('Assigned Job Cards'),add=find('Additional Time'),inc=find('Incentive Hours');let items=[job,assigned,add,inc].filter(Boolean);if(items.length&&!root.querySelector('.v84-action-grid')){let g=document.createElement('div');g.className='v84-action-grid';items[0].parentNode.insertBefore(g,items[0]);items.forEach((x,i)=>{x.classList.add('v84-action','v84-action-'+i);g.appendChild(x)})}
}
...
(function(){'use strict';
 ...
 window.renderEmployee=function(){
   ...
   const vehicleBrand=v=>{const s=String(v||'').toLowerCase();const brands=[
[/mercedes|benz|c[ -]?class|e[ -]?class|s[ -]?class/,'Mercedes-Benz','mercedes-benz.svg'],[/\\bbmw\\b/,'BMW','bmw.svg'],[/land rover|defender|discovery/,'Land Rover','land-rover.svg'],[/\\btoyota\\b|camry|corolla|land cruiser|prado|hilux|fortuner|hiace/,'Toyota','toyota.svg'],[/\\bnissan\\b|patrol|x-?trail|rogue|pathfinder|navara|urvan/,'Nissan','nissan-logo.svg'],[/\\binfiniti\\b/,'Infiniti','infiniti.svg'],[/\\bporsche\\b/,'Porsche','porsche.svg'],[/\\baudi\\b/,'Audi','audi.svg'],[/volkswagen|\\bvw\\b/,'Volkswagen','volkswagen.svg'],[/\\bvolvo\\b/,'Volvo','volvo.svg'],[/mitsubishi/,'Mitsubishi','mitsubishi.svg'],[/\\bmazda\\b/,'Mazda','mazda.svg'],[/\\bjeep\\b/,'Jeep','jeep.svg'],[/\\bhyundai\\b/,'Hyundai','hyundai-logo.svg'],[/\\bkia\\b/,'Kia','kia-logo.svg'],[/\\bgenesis\\b/,'Genesis','genesis-logo.svg'],[/\\bbyd\\b/,'BYD','byd-logo.svg'],[/\\bjetour\\b/,'Jetour','jetour.svg'],[/\\bgeely\\b/,'Geely','geely.svg'],[/\\blincoln\\b/,'Lincoln','lincoln.svg'],[/\\bchrysler\\b/,'Chrysler','chrysler.svg'],[/\\bisuzu\\b/,'Isuzu','isuzu.svg'],[/\\bram\\b/,'RAM','ram.svg'],[/\\bsuzuki\\b/,'Suzuki','suzuki.svg'],[/\\brenault\\b/,'Renault','renault.svg'],[/peugeot/,'Peugeot','peugeot.svg'],[/citro[eë]n/,'Citroën','citroen.svg'],[/\\bskoda\\b|škoda/,'Škoda','skoda.svg'],[/\\bbentley\\b/,'Bentley','bentley.svg'],[/rolls[ -]?royce/,'Rolls-Royce','rolls-royce.svg'],[/aston[ -]?martin/,'Aston Martin','aston-martin.svg'],[/\\bferrari\\b/,'Ferrari','ferrari.svg'],[/mclaren/,'McLaren','mclaren.svg'],[/\\bman\\b/,'MAN','man.svg'],[/great wall|\\bgwm\\b/,'GWM','gwm.svg']
...
`;document.head.appendChild(st);
   ...
   const brand='<div class="v75s-brand v80-compact-brand"><div class="v75s-logo">Z</div><div><h2>Zukait Time Track</h2></div><div class="v75s-menu">☰</div></div>';
   ...
   if(active&&aa){
     runningHtml+='<div class="v75s-hero"><div><div class="v75s-jno">JOB NUMBER : <b>'+esc(active.job)+'</b></div><div class="v75s-vehicle">'+esc(hold?'IDEAL TIME':(j.vehicle||'—'))+'</div><div class="v75s-reg">'+esc(hold?'Waiting / No Assigned Work':(j.reg||'—'))+'</div></div>'+(hold?'<div class="v75s-gear" style="margin-left:auto">◷</div>':vehicleBadge(j.vehicle))+'</div>'+
       '<div class="v75s-timepair"><div id="v80RunningGauge" class="v80-gauge v80-green"><div class="v80-gauge-inner"><span class="v80-gauge-title">RUNNING TIME</span><b id="v75sRunning">'+esc(running)+'</b><small id="v80RunningPct">0% USED</small><span class="v80-start">Started '+esc(startTime)+'</span></div></div><div id="v80RemainingGauge" class="v80-gauge v80-green"><div class="v80-gauge-inner"><span id="v80RemainingTitle" class="v80-gauge-title">'+(sg>0?'REMAINING TIME':'NO ALLOCATED TIME')+'</span><b id="currentRemaining">'+(sg>0?fm(remain):'—')+'</b><small id="v80RemainingPct">'+(sg>0?'100% LEFT':'SET TIME')+'</small></div></div></div>'+
   ...
   const month='<div class="card month-summary v75s-section"><div class="section-title"><h3>📅 This Month</h3><span class="pill">MONTHLY</span></div><div class="v81-month-grid">'+orb('v81-m1',String(normalDone.length),'Completed Jobs')+orb('v81-m2',eff==null?'—':eff.toFixed(1)+'%','Efficiency')+orb('v81-m3',fm(monthSuggested),'Suggested Time')+orb('v81-m4',fm(monthActual),'Actual Time')+orb('v81-m5',fm(Math.abs(monthRemaining)),monthRemaining>=0?'Remaining Time':'Over Suggested')+orb('v81-m6',fm(idealMin),'Ideal Time')+orb('v81-m7',fm(overtimeMin),'Overtime')+orb('v81-m8',fm(+monthInc.incentive||0),'Incentive Hours')+'</div></div>';
   ...
   root.innerHTML=brand+identity+runningHtml+allotted+month+leaveBox+finished+history;
