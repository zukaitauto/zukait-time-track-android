...
window.v132OpenSupervisorVehicleEdit=function(no){
 ...
 const body='<div class="notice"><b>Job Card:</b> '+v74JLE(j.no)+'</div><div class="grid"><label>Make<br><input id="v132SupMake" value="'+v74JLE(guessedMake)+'"></label><label>Model<br><input id="v132SupModel" value="'+v74JLE(guessedModel)+'"></label><label>Year<br>'+(j.year?'<input id="v132SupYear" value="'+v74JLE(j.year)+'" readonly title="Existing vehicle year">':'<input id="v132SupYear" inputmode="numeric" maxlength="4" placeholder="Year only if missing">')+'</label><label>Registration Number<br><input id="v132SupReg" value="'+v74JLE(j.reg||'')+'"></label></div><div class="notice small">Only vehicle details are changed. Job Card number, technicians and work-time history remain unchanged.</div><p><button class="green" onclick="v132SaveSupervisorVehicleEdit(\''+v74JLE(j.no)+'\')">SAVE CHANGES</button></p>';
...
/* V150 MANAGER SPARE PARTS PLACEHOLDER — future module, paired with Consumables. */
(function(){'use strict';
 window.v150OpenManagerSpareParts=function(){if(!window.me||me.role!=='Manager')return;const body='<div class="notice"><b>Spare Parts</b><div>This module is reserved for future use.</div></div>';if(typeof openModal==='function')return openModal('<div class="section-title"><h2>⚙️ Spare Parts</h2><button class="secondary" onclick="closeModal()">Close</button></div>'+body);alert('Spare Parts — Coming Soon')};
 function apply(){if(!window.me||me.role!=='Manager')return;const root=document.getElementById('managerView');if(!root)return;const cons=root.querySelector('.v109-manager-consumables,.v139-manager-consumables,[data-v144-consumables="1"]');if(!cons)return;let row=root.querySelector('.v150-manager-modules');if(!row){row=document.createElement('div');row.className='v150-manager-modules';cons.parentNode.insertBefore(row,cons);row.appendChild(cons)}let sp=row.querySelector('.v150-manager-spare');if(!sp){sp=document.createElement('button');sp.type='button';sp.className='v150-manager-spare';sp.onclick=window.v150OpenManagerSpareParts;sp.innerHTML='<span>⚙️ SPARE PARTS</span><b>›</b><small>Future module</small>';row.appendChild(sp)}}
 const st=document.createElement('style');st.id='v150ManagerModulesStyle';st.textContent='.v150-manager-modules{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin:0 0 12px}.v150-manager-modules>.v133-manager-leave,.v150-manager-modules>.v109-manager-consumables,.v150-manager-modules>.v139-manager-consumables{width:100%!important;margin:0!important;min-width:0}.v150-manager-spare{width:100%;min-width:0;margin:0;padding:13px 16px;border-radius:16px;border:1px solid #bdd5ef;background:linear-gradient(135deg,#eef6ff,#f8fbff);color:#173b63;display:grid;grid-template-columns:1fr auto;align-items:center;text-align:left;box-shadow:0 6px 14px #17324b16}.v150-manager-spare span{font-size:12px;font-weight:900}.v150-manager-spare b{grid-row:1/3;grid-column:2;font-size:27px}.v150-manager-spare small{font-weight:700;color:#526f8d}@media(max-width:360px){.v150-manager-modules{gap:7px}.v150-manager-spare,.v150-manager-modules>.v133-manager-leave{padding:11px 9px}.v150-manager-spare span{font-size:10px}}';document.head.appendChild(st);
...
function apply(){if(!window.me||me.role!=='Supervisor')return;const r=document.getElementById('supervisorView');if(!r)return;
 ...
 const quick=by('Quick\\s*Entry')||r.querySelector('.quick-entry');if(quick){quick.classList.add('v148-quick');const h=quick.querySelector('h3');if(h&&!h.dataset.v148){h.dataset.v148='1';h.innerHTML='⚡ '+h.innerHTML}quick.querySelectorAll('button').forEach(b=>{/CREATE.*ASSIGN|ASSIGN.*NOW/i.test(b.textContent||'')&&b.classList.add('v148-create')})}
 const assign=by('Assign\\s*\\/\\s*Update Job Card');if(assign){assign.classList.add('v148-assign');const h=assign.querySelector('h3');if(h&&!h.dataset.v148){h.dataset.v148='1';h.innerHTML='📋 '+h.innerHTML}assign.querySelectorAll('button').forEach(b=>{/ASSIGN|UPDATE/i.test(b.textContent||'')&&b.classList.add('v148-assign-btn')})}
...
'#supervisorView .v148-top{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:9px!important;margin-bottom:12px!important}'+
'#supervisorView .v148-top-card{min-height:74px!important;margin:0!important;padding:10px!important;border-radius:15px!important;display:flex!important;align-items:center!important;gap:8px!important;text-align:left!important;color:#172033!important;box-shadow:inset 0 1px 0 #fff,0 6px 14px #0f172218!important}'+
'#supervisorView .v148-req{background:#e8f3ff!important;border:1px solid #acd0f2!important}#supervisorView .v148-att{background:#fff3d7!important;border:1px solid #e6c36a!important}#supervisorView .v148-ot{background:#ffeadf!important;border:1px solid #efae8e!important}#supervisorView .v148-top-card b{color:#172033!important;font-size:11px!important}#supervisorView .v148-top-card small{display:block;color:#475569!important;font-size:8px!important}#supervisorView .v148-top-card strong{margin-left:auto;font-size:19px;color:#9a3412}'+
'#supervisorView .v148-quick{background:linear-gradient(145deg,#e5f8ff,#f4fcff)!important;border:1px solid #99d8eb!important}#supervisorView .v148-quick h3{color:#075985!important}#supervisorView .v148-assign{background:linear-gradient(145deg,#f2ebff,#fbf8ff)!important;border:1px solid #cbb7ef!important}#supervisorView .v148-assign h3{color:#5b21b6!important}'+
'#supervisorView .v148-create{background:linear-gradient(135deg,#0e7490,#0284c7)!important;color:#fff!important;border:1px solid #0369a1!important;font-weight:900!important;box-shadow:0 5px 0 #075985,0 8px 14px #0284c738!important}#supervisorView .v148-assign-btn{background:linear-gradient(135deg,#7c3aed,#6d28d9)!important;color:#fff!important;border:1px solid #5b21b6!important;font-weight:900!important;box-shadow:0 5px 0 #4c1d95,0 8px 14px #7c3aed38!important}'+
'#supervisorView .v148-ideal,#supervisorView .v148-modules{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:10px!important;margin:12px 0!important}#supervisorView .v148-3d{margin:0!important;min-width:0!important;border-radius:16px!important;box-shadow:inset 0 2px 3px #fff,0 7px 0 #00000012,0 11px 20px #0f17221c!important}'+
'#supervisorView .v148-modules .v148-module{min-height:86px!important;width:100%!important;padding:14px!important;display:grid!important;grid-template-columns:auto 1fr auto!important;align-items:center!important;gap:10px!important;text-align:left!important;color:#172033!important}#supervisorView .v148-spare{background:linear-gradient(145deg,#e8f7f0,#d9efe5)!important;border:1px solid #a6d3bc!important}#supervisorView .v148-module b{display:block;color:#172033!important;font-size:13px}#supervisorView .v148-module small{display:block;color:#475569!important;font-size:9px;margin-top:3px}'+
