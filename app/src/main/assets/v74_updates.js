...
(function(){'use strict';
 function apply(){
  ...
  const quick=root.querySelector('.quick-entry');if(quick){quick.querySelectorAll('p.muted,.time-hint').forEach(x=>x.remove());const g=quick.querySelector('.grid');if(g){g.classList.add('v119-grid');const a=[g.querySelector('#newNo')?.closest('label'),g.querySelector('#newVehicle')?.closest('label'),g.querySelector('#newYear')?.closest('label'),g.querySelector('#se')?.closest('label'),g.querySelector('#st')?.closest('label'),quick.querySelector('.v107-create-assign,button[onclick*="createAndAssignJob"]')].filter(Boolean);const names=['📋 Job Card ','🚗 Vehicle Make / Model ','📅 Year ','👨‍🔧 Technician ','⏱ Time '];a.slice(0,5).forEach((x,n)=>{x.childNodes[0].textContent=names[n]});if(a[5]){a[5].classList.add('v119-primary');a[5].innerHTML='✓ Create Job + Assign'}a.forEach(x=>g.appendChild(x))}}
  ...
  root.querySelectorAll('.v118-action-grid,.v84-action-grid').forEach(x=>x.remove());let old=[...root.querySelectorAll('details.card')].find(x=>/Additional Action History/i.test(x.textContent||''));if(old)old.remove();let s=root.querySelector('#v119Actions');if(!s){s=document.createElement('section');s.id='v119Actions';s.className='card v119-actions';root.appendChild(s)}s.innerHTML='<h3>Additional Action History</h3><div class="v119-actions-grid"><button onclick="openSupervisorJobCardList()">📋<b>Job Card Details</b><small>Full information</small></button><button onclick="openSupervisorAssignedWindow()">🗂<b>Assigned Job Cards</b><small>Open assignments</small></button><button onclick="manualAdditionalTime()">⏱<b>Additional Time</b><small>Add allocated time</small></button></div>';
