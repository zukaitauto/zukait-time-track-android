...
function apply(){if(!window.me||me.role!=='Supervisor')return;const r=document.getElementById('supervisorView');if(!r)return;
 ...
 let top=r.querySelector('.v148-top');if(!top){top=document.createElement('div');top.className='v148-top';const id=r.querySelector('.v91-role-identity');(id||r.firstElementChild).insertAdjacentElement(id?'afterend':'beforebegin',top)}
...
document.head.appendChild(st);const old=window.render;window.render=function(){const z=typeof old==='function'?old.apply(this,arguments):undefined;setTimeout(apply,0);setTimeout(apply,200);return z};setTimeout(apply,0);window.v148SupervisorApply=apply;
})();
