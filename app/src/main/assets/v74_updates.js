...
function v84TechState(u){let s=activeSession(u.id),st='Available';if(s){let ot=0;try{ot=sessionOvertimeMinutes(s,Date.now())}catch(_){}st=ot>0?'Overtime':(s.paused?'Paused':'Working')}return{session:s,status:st}}
function v84TechnicianBoard(){let ds=[['Denter','DENTING','🛠️','v84-denter'],['Painter','PAINTING','🎨','v84-painter'],['Mechanic','MECHANICAL','⚙️','v84-mechanic']];return '<div class="card v84-tech-board v92-tech-board"><div class="v92-tech-title"><div><h3>👷 Technician Board</h3><small>Live workshop status · tap a department</small></div><span class="v92-live-dot">● LIVE</span></div><div class="v84-depts v92-tech-depts">'+ds.map(d=>{let t=(users||[]).filter(u=>u.role==='Employee'&&u.department===d[0]),working=t.filter(u=>!!v84TechState(u).session).length;return '<button class="v84-dept v92-tech-dept '+d[3]+'" onclick="v84OpenDept(\''+d[0]+'\')"><span class="v92-dept-icon">'+d[2]+'</span><b>'+d[1]+'</b><strong>'+working+' <em>/ '+t.length+'</em></strong><small>WORKING / TOTAL</small><i>Tap to view ›</i></button>'}).join('')+'</div></div>'}
...
function v84SupervisorPanels(root){
 ...
 if(req){let oldRow=req.closest('.v88-alert-row,.v84-alert-row');if(oldRow&&att&&oldRow.contains(att))oldRow.parentNode.insertBefore(att,oldRow.nextSibling);let row=root.querySelector('.v92-supervisor-top');if(!row){row=document.createElement('div');row.className='v92-supervisor-top';const identity=root.querySelector('.v91-role-identity');(identity||root.firstChild).insertAdjacentElement(identity?'afterend':'beforebegin',row)}let available=(users||[]).filter(u=>u.role==='Employee'&&!v84TechState(u).session).length;req.className='v92-top-card v92-request-card clickable';req.onclick=()=>window.openSupervisorRequestsWindow();row.appendChild(req);let av=row.querySelector('.v92-available-card');if(!av){av=document.createElement('button');av.className='v92-top-card v92-available-card clickable';row.appendChild(av)}av.onclick=()=>window.v92OpenAvailableWorkers();av.innerHTML='<span class="v92-top-icon">👷</span><span><b>AVAILABLE WORKERS</b><small>Ready for assignment</small></span><strong>'+available+'</strong>';if(oldRow&&oldRow.isConnected&&!oldRow.children.length)oldRow.remove()}
 ...
 if(job&&job.isConnected){job.classList.add('clickable');job.setAttribute('onclick','openSupervisorJobCardList()');job.innerHTML='<div class="section-title"><h3>📋 Job Card List</h3><span class="pill">Click to open</span></div><div class="small muted">All Job Cards · filters · sorting · export</div>'}
 ...
 const items=[job,assigned,add,inc].filter(x=>x&&x.isConnected);
...
function v91RoleHeader(role){
 ...
 const row=document.createElement('div');row.className='v91-role-identity';row.innerHTML='<b>'+E(me.name)+' · '+E(dept||role)+'</b><span class="v91-role-online"><i></i>ONLINE</span><button class="v91-role-menu" onclick="'+(role==='Supervisor'?'v65OpenAccount()':'v65OpenAccount()')+'" aria-label="Open account menu">☰</button>';
