...
function v84TechnicianBoard(){let ds=[['Denter','DENTING','🛠️','v84-denter'],['Painter','PAINTING','🎨','v84-painter'],['Mechanic','MECHANICAL','⚙️','v84-mechanic']];return '<div class="card v84-tech-board"><h3>👷 Technician Board</h3><div class="v84-depts">'+ds.map(d=>{let t=(users||[]).filter(u=>u.role==='Employee'&&u.department===d[0]),working=t.filter(u=>!!v84TechState(u).session).length;return '<button class="v84-dept '+d[3]+'" onclick="v84OpenDept(\''+d[0]+'\')"><span>'+d[2]+'</span><b>'+d[1]+'</b><strong>'+working+'/'+t.length+'</strong><small>WORKING / TOTAL</small></button>'}).join('')+'</div></div>'}
...
`;document.head.appendChild(st);
   ...
   const repeatJobs=all.filter(a=>a.job!==H&&a.rework===true).slice().sort((a,b)=>(b.assignedAt||0)-(a.assignedAt||0));
   const repeatList='<div class="card v75s-section"><div class="section-title"><h3>🔁 Repeat Jobs</h3><span class="pill">'+repeatJobs.length+'</span></div>'+(repeatJobs.length?'<div class="v75s-history"><table><tr><th>Job</th><th>Vehicle</th><th>Allocated</th><th>Actual</th><th>Status</th></tr>'+repeatJobs.slice(0,20).map(a=>{const x=jj(a.job),st=a.completed?'Finished':(active&&active.assignmentId===a.id?'Running':a.cancelled?'Cancelled':'Assigned');return '<tr><td><b>'+esc(a.job)+'</b></td><td>'+esc(x.vehicle||'—')+'</td><td>'+fm(a.suggested)+'</td><td>'+fm(actual(a))+'</td><td><b>'+esc(st)+'</b></td></tr>'}).join('')+'</table></div>':'<p class="muted">No repeat jobs assigned.</p>')+'</div>';
   ...
   root.innerHTML=brand+identity+runningHtml+allotted+month+leaveBox+finished+repeatList+history;
