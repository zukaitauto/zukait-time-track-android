...
window.v74JobListRender=function(){
 ...
 const listView=(localStorage.getItem('zukaitJobListView')||'cards')==='list';el.innerHTML=rows.length?'<div class="v132-jc-grid '+(listView?'v162-list-view':'')+'">'+rows.map(x=>{let names=[...new Set(x.aa.map(a=>v74JLP(a.emp).name))],[s,cl]=statusOf(x),sg=x.aa.reduce((n,a)=>n+(+a.suggested||0),0),ac=x.aa.reduce((n,a)=>{try{return n+totalForAssignment(a)}catch(_){return n}},0),rem=Math.max(0,sg-ac);return '<button type="button" class="v132-jc-card '+cl+'" onclick="v132OpenSupervisorJobFull(\''+v74JLE(x.j.no)+'\')"><div class="v132-jc-top"><b>'+v74JLE(x.j.no)+'</b><span>'+s+'</span></div><h3>'+v74JLE(x.j.vehicle||'—')+'</h3><p>'+v74JLE(x.j.reg||'—')+'</p><div class="v132-jc-tech">'+v74JLE(names.join(', ')||'Unassigned')+'</div><div class="v132-jc-mini"><div>Suggested<b>'+fmt(sg)+'</b></div><div>'+(cl==='exceeded'?'Exceeded':'Remaining')+'<b>'+fmt(cl==='exceeded'?Math.max(0,ac-sg):rem)+'</b></div></div></button>'}).join('')+'</div>':'<div class="notice">No Job Cards match the selected filters.</div>';
};
