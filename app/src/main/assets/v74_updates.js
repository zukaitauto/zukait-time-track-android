...
(function(){'use strict';
 ...
 const baseCreate=window.createAndAssignJob;window.createAndAssignJob=function(){const no=document.getElementById('newNo')?.value.trim().toUpperCase()||'',v=document.getElementById('newVehicle')?.value.trim()||'',reg=document.getElementById('newReg')?.value.trim().toUpperCase()||'',emp=document.getElementById('se')?.value||'',s=typeof parseWorkMinutes==='function'?parseWorkMinutes(document.getElementById('st')?.value||''):NaN;if(!no||!v||!reg||!emp||!user(emp)||user(emp).role!=='Employee'||!Number.isFinite(s)||s<1)return alert('Enter Job Card, Make / Model, Registration Number, technician and valid time.');if(job(no))return alert('Job Card Number already exists.');state.jobs.push({no,vehicle:v,reg:reg,status:'Open',createdBy:me.id,createdAt:now()});const a=window.assignJobCore(no,emp,s);if(!a){state.jobs=state.jobs.filter(j=>j&&j.no!==no);return}setLastAction('Created and assigned '+no+' to '+user(emp).name);save();render()};
 function apply(){
  if(!me||me.role!=='Supervisor')return;
  ...
  const quick=root.querySelector('.quick-entry');if(quick){quick.querySelectorAll('p.muted,.time-hint').forEach(x=>x.remove());const g=quick.querySelector('.grid');if(g){g.classList.add('v119-grid');const no=g.querySelector('#newNo')?.closest('label'),veh=g.querySelector('#newVehicle')?.closest('label'),reg=g.querySelector('#newReg')?.closest('label'),tech=g.querySelector('#se')?.closest('label'),time=g.querySelector('#st')?.closest('label');const a=[no,veh,reg,tech,time,quick.querySelector('.v107-create-assign,button[onclick*="createAndAssignJob"]')].filter(Boolean);const names=['📋 Job Card ','🚗 Make / Model ','🔢 Registration Number ','👨‍🔧 Technician ','⏱ Time '];a.slice(0,5).forEach((x,n)=>{x.childNodes[0].textContent=names[n]});if(a[5]){a[5].classList.add('v119-primary');a[5].innerHTML='✓ Create Job + Assign'}a.forEach(x=>g.appendChild(x))}}
