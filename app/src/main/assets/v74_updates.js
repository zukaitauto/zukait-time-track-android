...
function v91RoleHeader(role){
 ...
 // Supervisor owns one identity/status row only. Hide every legacy/global Online source.
 if(role==='Supervisor'){
...
(function(){'use strict';
 ...
 const baseCreate=window.createAndAssignJob;window.createAndAssignJob=function(){const y=document.getElementById('newYear');if(!y)return typeof baseCreate==='function'?baseCreate():undefined;const no=document.getElementById('newNo')?.value.trim().toUpperCase()||'',v=document.getElementById('newVehicle')?.value.trim()||'',yr=y.value.trim(),emp=document.getElementById('se')?.value||'',s=typeof parseWorkMinutes==='function'?parseWorkMinutes(document.getElementById('st')?.value||''):NaN;if(!no||!v||!/^\\d{4}$/.test(yr)||!emp||!Number.isFinite(s)||s<1)return alert('Enter Job Card, vehicle make/model, 4-digit model year, technician and valid time.');if(job(no))return alert('Job Card Number already exists.');state.jobs.push({no,vehicle:v,modelYear:yr,reg:'',status:'Open',createdBy:me.id,createdAt:now()});state.assign.push({id:uid(),job:no,emp,suggested:s,completed:false,rework:false,assignedBy:me.id,assignedAt:now()});setLastAction('Created and assigned '+no+' to '+user(emp).name);save();render()};
 function apply(){
  if(!me||me.role!=='Supervisor')return;
  ...
  const quick=root.querySelector('.quick-entry');if(quick){quick.querySelectorAll('p.muted,.time-hint').forEach(x=>x.remove());const g=quick.querySelector('.grid');if(g){g.classList.add('v119-grid');const no=g.querySelector('#newNo')?.closest('label'),veh=g.querySelector('#newVehicle')?.closest('label'),reg=g.querySelector('#newReg')?.closest('label'),tech=g.querySelector('#se')?.closest('label'),time=g.querySelector('#st')?.closest('label');if(reg){let y=document.createElement('label');y.innerHTML='📅 Year <br><input id="newYear" inputmode="numeric" maxlength="4" placeholder="">';reg.replaceWith(y)}const year=g.querySelector('#newYear')?.closest('label'),a=[no,veh,year,tech,time,quick.querySelector('.v107-create-assign,button[onclick*="createAndAssignJob"]')].filter(Boolean);const names=['📋 Job Card ','🚗 Vehicle Make / Model ','📅 Year ','👨‍🔧 Technician ','⏱ Time '];a.slice(0,5).forEach((x,n)=>{x.childNodes[0].textContent=names[n]});if(a[5]){a[5].classList.add('v119-primary');a[5].innerHTML='✓ Create Job + Assign'}a.forEach(x=>g.appendChild(x))}}
  const assign=[...root.querySelectorAll('.card')].find(x=>/Assign\s*\/\s*Update Job Card/i.test(x.querySelector('h3')?.textContent||''));if(assign){assign.classList.add('v119-assign');assign.querySelectorAll('p.muted,.time-hint').forEach(x=>x.remove());let g=assign.querySelector('.v112-assign-grid,.grid');if(g){g.classList.add('v119-grid');const j=g.querySelector('#sj')?.closest('label'),t=g.querySelector('#se2')?.closest('label'),tm=g.querySelector('#st2')?.closest('label'),b=assign.querySelector('.v107-assign,button[onclick*="assignJobExisting"]');if(j)j.childNodes[0].textContent='🔎 Job Card Search ';if(t)t.childNodes[0].textContent='👨‍🔧 Technician ';if(tm)tm.childNodes[0].textContent='⏱ Allocated Time ';if(b){b.classList.add('v119-primary');b.innerHTML='✓ Assign / Update'}let id=root.querySelector('#v112ID001Quick');if(!id){id=document.createElement('button');id.id='v112ID001Quick'}id.className='v119-id';id.innerHTML='<b>◷ ID001 Assign</b><small>Ideal Time</small>';id.onclick=window.v112OpenID001Quick;let h=g.querySelector('#v119ID001History');if(!h){h=document.createElement('button');h.id='v119ID001History'}h.className='v119-id';h.innerHTML='<b>▤ ID001 History</b><small>Details · Print</small>';h.onclick=()=>window.v753OpenID001Report?.();[j,t,tm,b,id,h].filter(Boolean).forEach(x=>g.appendChild(x))}}
  root.querySelectorAll('.v118-action-grid,.v84-action-grid').forEach(x=>x.remove());let old=[...root.querySelectorAll('details.card')].find(x=>/Additional Action History/i.test(x.textContent||''));if(old)old.remove();let s=root.querySelector('#v119Actions');if(!s){s=document.createElement('section');s.id='v119Actions';s.className='card v119-actions';root.appendChild(s)}s.innerHTML='<h3>Additional Action History</h3><div class="v119-actions-grid"><button onclick="openSupervisorJobCardList()">📋<b>Job Card Details</b><small>Full information</small></button><button onclick="v119OpenAssignedJobs()">🗂<b>Assigned Job Cards</b><small>Open assignments</small></button><button onclick="manualAdditionalTime()">⏱<b>Additional Time</b><small>Add allocated time</small></button></div>';
...
(function(){'use strict';
 ...
 function inject(){if(!me)return;const root=document.getElementById(me.role==='Supervisor'?'supervisorView':me.role==='Manager'?'managerView':'employeeView');if(!root)return;
  if(me.role==='Supervisor'){const v=document.getElementById('newVehicle');if(v&&!document.getElementById('v131NewBrand')){const lab=document.createElement('label');lab.className='v131-brand-field';lab.innerHTML='🏷 Vehicle Brand<br><select id="v131NewBrand">'+opts('')+'</select>';v.closest('label')?.insertAdjacentElement('afterend',lab);v.addEventListener('input',()=>{const s=document.getElementById('v131NewBrand');if(s&&!s.value)s.value=infer(v.value)})}}
 }
 const oldCreate=window.createAndAssignQuick||window.quickCreateAssign; // preserve existing authority if present
 const wrapCreate=name=>{const fn=window[name];if(typeof fn!=='function'||fn.v131)return;const w=function(){const no=document.getElementById('newNo')?.value?.trim()?.toUpperCase(),brand=document.getElementById('v131NewBrand')?.value||infer(document.getElementById('newVehicle')?.value);const r=fn.apply(this,arguments);if(no){const j=(state.jobs||[]).find(x=>x&&x.no===no);if(j&&!j.brand){j.brand=brand;try{save()}catch(_){}}}return r};w.v131=true;window[name]=w};['createJob','createAndAssign','createAndAssignQuick','quickCreateAssign'].forEach(wrapCreate);
...
(function(){'use strict';
 ...
 window.v133OpenManagerLeave=function(){if(!me||me.role!=='Manager')return;const t=today(),m=month(),days=m.reduce((n,l)=>n+(l.period==='FULL'?1:.5),0);openModal('<div class="section-title"><h2>🗓 Leave Control</h2><button class="secondary" onclick="closeModal()">Close</button></div><div class="v133-leave-summary"><div><span>ON LEAVE TODAY</span><b>'+new Set(t.map(x=>x.emp)).size+'</b></div><div><span>THIS MONTH</span><b>'+days.toFixed(1).replace('.0','')+' days</b></div></div><h3>Today</h3>'+table(t)+'<h3>This Month</h3>'+table(m))};
 function apply(){if(!me||me.role!=='Manager')return;const root=document.getElementById('managerView');if(!root)return;const t=today(),count=new Set(t.map(x=>String(x.emp))).size;let btn=root.querySelector('.v133-manager-leave');if(!btn){btn=document.createElement('button');btn.className='v133-manager-leave';btn.onclick=window.v133OpenManagerLeave;const perf=root.querySelector('.v123-manager-performance');(perf?.parentNode||root).insertBefore(btn,perf?perf.nextSibling:root.firstChild)}btn.innerHTML='<span>🗓 TODAY LEAVE</span><b>'+count+'</b><small>Tap for today + this month details</small>'}
 const prev=window.render;window.render=function(){const r=typeof prev==='function'?prev.apply(this,arguments):undefined;setTimeout(apply,0);return r};
