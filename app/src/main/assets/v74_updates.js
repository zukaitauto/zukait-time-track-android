...
function v84SupervisorPanels(root){
 ...
 const job=find('Job Card Details'),assigned=find('Assigned Job Cards'),add=find('Additional Time'),inc=find('Incentive Hours');
 // Supervisor lower controls: one Job Card Details only. Assigned Job Cards and Additional Time open in modal boxes.
 if(job&&job.isConnected){job.classList.add('clickable');job.setAttribute('onclick','openSupervisorJobCardList()');job.innerHTML='<div class="section-title"><h3>📋 Job Card Details</h3><span class="pill">Click to open</span></div><div class="small muted">Full job information · search · status · working time</div>'}
...
function v91RoleHeader(role){
 ...
 // Supervisor owns one identity row only; hide the old global role/status strip to prevent the duplicate header seen on device.
 if(role==='Supervisor'&&gh)gh.style.setProperty('display','none','important');
}
