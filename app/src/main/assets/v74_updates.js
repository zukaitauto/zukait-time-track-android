...
(function(){'use strict';
 ...
 function apply(){if(!me||me.role!=='Manager')return;const root=document.getElementById('managerView');if(!root)return;const t=today(),count=new Set(t.map(x=>String(x.emp))).size;
   ...
   const btn=document.createElement('button');btn.className='v133-manager-leave v109-manager-leave';btn.onclick=window.v133OpenManagerLeave;btn.innerHTML='<span>🗓 ON LEAVE</span><b>'+count+'</b><small>Today · tap for today + this month details</small>';
