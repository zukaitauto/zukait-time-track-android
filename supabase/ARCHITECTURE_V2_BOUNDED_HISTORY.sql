...
begin
  ...
  if found then v_inserted:=true; else select * into v from public.workshop_v2_events e where e.event_id=p_event_id; end if;
...
begin
  ...
  if found then v_inserted:=true; else select * into v_row from public.workshop_v2_events e where e.event_id=p_event_id; end if;
  if v_row.entity_id<>p_entity_id or v_row.event_type<>p_event_type or coalesce(v_row.actor_id,'')<>coalesce(p_actor_id,'') or coalesce(v_row.device_id,'')<>coalesce(p_device_id,'') or coalesce(v_row.payload,'{}'::jsonb)<>coalesce(p_payload,'{}'::jsonb) then raise exception 'event_id_conflict'; end if;
  return query select v_row.event_id,v_row.server_time,v_row.revision,v_inserted;
end;$;
revoke all on function public.zukait_v2_commit_event(text,text,text,text,text,timestamptz,bigint,jsonb) from public,anon,authenticated;
grant execute on function public.zukait_v2_commit_event(text,text,text,text,text,timestamptz,bigint,jsonb) to service_role;
