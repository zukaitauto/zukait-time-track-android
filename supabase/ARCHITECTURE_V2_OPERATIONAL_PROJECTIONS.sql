...
declare cur public.workshop_v2_work_sessions%rowtype; mins integer:=0; kindv text; emp text; job text;
begin
 emp:=coalesce(p_payload->>'employeeId',''); job:=coalesce(p_payload->>'jobCard',p_payload->>'job',''); kindv:=case when p_event_type like 'ID001%' then 'ID001' else 'WORK' end;
 ...
 if p_event_type in ('WORK_START','ID001_START') then
   ...
   insert into public.workshop_v2_work_sessions(session_id,assignment_id,job_card,employee_id,kind,started_at,status,suggested_minutes,repeat_minutes,last_event_id,revision)
   values(p_entity_id,p_entity_id,job,emp,kindv,coalesce(p_event_time,now()),'ACTIVE',case when kindv='ID001' then 0 else greatest(coalesce((p_payload->>'suggestedMinutes')::integer,0),0) end,greatest(coalesce((p_payload->>'repeatMinutes')::integer,0),0),p_event_id,coalesce(p_revision,0))
   on conflict(session_id) do update set started_at=excluded.started_at,ended_at=null,status='ACTIVE',last_event_id=excluded.last_event_id,revision=excluded.revision,updated_at=now();
 elsif not found then raise exception 'work_session_missing';
 elsif p_event_type='WORK_PAUSE' then update public.workshop_v2_work_sessions set status='PAUSED',last_event_id=p_event_id,revision=p_revision,updated_at=now() where session_id=p_entity_id;
 elsif p_event_type='WORK_RESUME' then update public.workshop_v2_work_sessions set status='ACTIVE',last_event_id=p_event_id,revision=p_revision,updated_at=now() where session_id=p_entity_id;
 elsif p_event_type in ('WORK_FINISH','ID001_STOP') then
   mins:=greatest(0,floor(extract(epoch from (coalesce(p_event_time,now())-cur.started_at))/60)::integer);
   update public.workshop_v2_work_sessions set ended_at=coalesce(p_event_time,now()),status=case when kind='ID001' then 'STOPPED' else 'FINISHED' end,actual_minutes=mins,last_event_id=p_event_id,revision=p_revision,updated_at=now() where session_id=p_entity_id;
 end if;
end;$;
